...
static void alloc_bmmu_buf(struct vdec_h264_hw_s *hw, int i, bool field_flag)
{
	...
	if (vdec->vdata != NULL) {
		...
		if (index >= 0) {
			pic->user_data_buf[0] = vdec->vdata->data[index].user_data_buf;
			if (field_flag == 1)
				pic->user_data_buf[1] = pic->user_data_buf[0] + SEI_ITU_DATA_SIZE;
...
static void hevc_mcr_config_mc_ref(struct vdec_h264_hw_s *hw)
{
	...
	for (i = 0; i < (unsigned int)(pSlice->listXsize[0]); i++) {
		...
		if (ref == NULL)
			return;
		WRITE_VREG(CURR_CANVAS_CTRL, hw->buffer_spec[ref->buf_spec_num].canvas_pos << 24);
		...
		WRITE_VREG(HEVCD_MPP_ANC_CANVAS_ACCCONFIG_ADDR,
					(hw->buffer_spec[ref->buf_spec_num].canvas_pos & 0x3f) << 8);
...
static void hevc_sao_set_pic_buffer(struct vdec_h264_hw_s *hw,
	...
	WRITE_VREG(CURR_CANVAS_CTRL, hw->buffer_spec[pic->buf_spec_num].canvas_pos << 24);
...
static void hevc_set_unused_4k_buff_idx(struct vdec_h264_hw_s *hw,
		u32 buf_spec_num)
{
	hw->hevc_cur_buf_idx = buf_spec_num & 0xff;
	dpb_print(DECODE_ID(hw),
...
static int alloc_one_buf_spec(struct vdec_h264_hw_s *hw, int i, bool field_flag)
{
	struct vdec_s *vdec = hw_to_vdec(hw);
	if (hw->mmu_enable) {
		...
		hw->buffer_spec[i].canvas_config[0].block_mode =
			hw->canvas_mode;
		hw->buffer_spec[i].canvas_config[0].endian = 0;
		...
		hw->buffer_spec[i].canvas_config[1].block_mode =
				hw->canvas_mode;
		hw->buffer_spec[i].canvas_config[1].endian = 0;
...
static int alloc_one_buf_spec_from_queue(struct vdec_h264_hw_s *hw, int idx)
{
	...
	bs->cma_alloc_addr = (unsigned long)aml_buf;
	dpb_print(DECODE_ID(hw), PRINT_FLAG_V4L_DETAIL,
	...
	if (aml_buf->num_planes == 1) {
		y_addr = aml_buf->planes[0].addr;
		c_addr = aml_buf->planes[0].addr + aml_buf->planes[0].offset;
		aml_buf->planes[0].bytes_used = aml_buf->planes[0].length;
	} else if (aml_buf->num_planes == 2) {
		y_addr = aml_buf->planes[0].addr;
		c_addr = aml_buf->planes[1].addr;
		aml_buf->planes[0].bytes_used = aml_buf->planes[0].length;
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_V4L_DETAIL,
		"[%d] %s(), y_addr: %x, size: %u\n",
		ctx->id, __func__, y_addr, aml_buf->planes[0].length);
	dpb_print(DECODE_ID(hw), PRINT_FLAG_V4L_DETAIL,
		"[%d] %s(), c_addr: %x, size: %u\n",
		ctx->id, __func__, c_addr, aml_buf->planes[1].length);
...
static int v4l_find_buffer_spec_idx(struct vdec_h264_hw_s *hw, unsigned int v4l_index)
{
	int i;

	for (i = 0; i < BUFSPEC_POOL_SIZE; i++) {
		if (hw->buffer_wrap[i] == v4l_index)
			return i;
	}
	return -1;
}
		...
		case V4L_CAP_BUFF_IN_M2M:
			...
			if (rt >= 0) {
				...
				if (!alloc_one_buf_spec_from_queue(hw, rt)) {
					...
					hw->buffer_wrap[rt] = aml_buf->index;
					idx = rt;
				}
			}
			break;
...
void h264_mmu_box_free(struct kref *kref)
{
	struct vdec_h264_hw_s *hw = container_of(kref, struct vdec_h264_hw_s, box_ref);

	if (hw->mmu_box) {
		dpb_print(DECODE_ID(hw), PRINT_FLAG_MMU_DETAIL,"h264 free mmu box\n");
	...
	for (i = 0; i < BUFSPEC_POOL_SIZE; i++) {
		if (hw->buffer_spec[i].used == 4 ||
			...
			if (hw->buffer_spec[i].alloc_header_addr) {
				...
				spin_unlock_irqrestore
					(&hw->bufspec_lock, flags);
				if (kref_read(&hw->box_ref))
					kref_put(&hw->box_ref, h264_mmu_box_free);
			}
		}
	}
	if (dealloc_flag &&
...
unsigned char have_free_buf_spec(struct vdec_s *vdec)
{
	...
	bool field_flag = get_field(hw, hw->dpb.mSPS.frame_mbs_only_flag);
	...
	if (index >= 0) {
		...
		if (max_alloc_buf_count == 0 ||
			...
			if (alloc_one_buf_spec(hw, index, field_flag) >= 0)
				ret = 1;
		}
...
static bool inline is_open_h264_mmu(struct vdec_h264_hw_s *hw, bool field_flag)
{
	...
	if (field_flag) {
		dpb_print(DECODE_ID(hw), 0, "h264 interlace field_flag is %d\n", field_flag);
		return false;
	}
...
static int clear_mmu_config(struct vdec_h264_hw_s *hw)
{
	struct vdec_s *vdec = hw_to_vdec(hw);
	...
	if (!is_vdec_hevc_combine())
		amhevc_stop();
	amvdec_stop();
	hw->canvas_mode = vdec->canvas_mode;
...
int set_mmu_config(struct vdec_h264_hw_s *hw)
{
	...
	if (!(hw->mask & CORE_MASK_HEVC) && !is_vdec_hevc_combine()) {
		vdec_core_release(vdec, hw->mask);
		if (is_support_dual_core())
	...
	dpb_print(DECODE_ID(hw), 0, "%s mmu_enable %d double_write_mode 0x%x\n",
		__func__, hw->mmu_enable, hw->double_write_mode);

	return 0;
}

static inline bool pre_proc_for_mmu(struct vdec_h264_hw_s *hw, bool field_flag)
{
	if (is_open_h264_mmu(hw, field_flag)) {
		if (!hw->mmu_enable) {
			set_mmu_config(hw);
			hw->mmu_cfg_changed = true;
			/* need parse sps again,so set init_flag */
			if (hw->seq_info2 == 0)
				hw->init_flag = 0;
			return true;
		}
	...
	u32 param1, u32 param2, u32 param3, u32 param4, bool buffer_reset_flag, bool reset_flags)
{
	...
	field_flag = get_field(hw, (hw->seq_info >> 15) & 0x01);
	...
	if (((seq_info2 != 0 &&
		...
		h264_reconfig(hw);
		if (pre_proc_for_mmu(hw, field_flag))
			return 0;
	} else {
	...
	if (hw->config_bufmgr_done == 0) {
		...
		if (!hw->mmu_enable) {
			...
			if (hw->is_used_v4l) {
				...
				if ((i != -1) && alloc_one_buf_spec(hw, i, field_flag) >= 0) {
			...
			if (!reset_flags) {
				for (i = 0; i < active_buffer_spec_num; i++) {
					if (is_vdec_hevc_combine())
				...
				for (j = 0; j < active_buffer_spec_num; j++) {
					i = get_buf_spec_by_canvas_pos(hw, j);
					alloc_bmmu_buf(hw, i, field_flag);
				}
			...
			if (ret >= 0 && hw->decode_pic_count == 0) {
				...
				for (j = 1; j < buf_cnt; j++) {
					...
					if (hw->is_used_v4l) {
						...
						if ((-1 != i) && alloc_one_buf_spec(hw, i, field_flag) >= 0) {
							config_decode_canvas(hw, i);
						} else {
							mutex_unlock(&vmh264_mutex);
							return -1;
	...
	case SEI_USER_DATA:
		if (enable_itu_t35 && parse_ud_flag) {
			...
			if (p_sei[0] == 0xB5
				...
				if (dpb_is_debug(DECODE_ID(hw),
					PRINT_FLAG_SEI_DETAIL)) {
					dpb_print(DECODE_ID(hw), 0, "%s: hdr sei_itu_data_len %d\n", __func__, hw->sei_itu_data_len);
	...
	if (dec_dpb_status == H264_CONFIG_REQUEST) {
		...
		if (p_H264_Dpb->mVideo.dec_picture) {
			...
			if ((!I_flag && (frame_num_gap == FrameNumGap_Normal) && (!p_H264_Dpb->long_term_reference_flag)
				&& input_frame_based(vdec)) || (frame_num_gap == FrameNumGap_Loop)) {
						...
						u32 index)
{
	...
	if (p != NULL && p->buf_spec_num >= 0) {
		...
		if (pic->user_data_buf[index] != NULL) {
			memset(pic->user_data_buf[index], 0, SEI_ITU_DATA_SIZE);
			if (hw->sei_itu_data_len < SEI_ITU_DATA_SIZE &&
				(hw->sei_itu_data_len > 0)) {
				memcpy(pic->user_data_buf[index], hw->sei_itu_data_buf,
					hw->sei_itu_data_len);
		...
		ud_param->pbuf_addr = pic->user_data_buf[index];
	...
	if (dpb_is_debug(DECODE_ID(hw), PRINT_FLAG_SEI_DETAIL)) {
		dpb_print(DECODE_ID(hw), 0, "%s: sei_itu_data_len %d\n", __func__, hw->sei_itu_data_len);
		for (i = 0; i < hw->sei_itu_data_len; i++) {
			...
			if (!v4l_res_change(hw, param1, param2, param3, param4)) {
				if (!hw->v4l_params_parsed) {
					...
					param2, param3, param4, false, false) < 0) {
						...
						hw->csd_restore_flag = true;
						hw->reset_bufmgr_flag = 1;
						dpb_print(DECODE_ID(hw), 0, "set parameters error, init_flag: %u, csd_error_flag %d\n",
	...
	*/
	...
	for (i = 0; i < BUFSPEC_POOL_SIZE; i++) {
		...
		*/
		...
		if (hw->buffer_spec[i].used == 4 &&
			hw->buffer_spec[i].vf_ref != 0 &&
			(hw->buffer_spec[i].cma_alloc_addr || hw->buffer_spec[i].alloc_header_addr)) {
			hw->buffer_spec[i].used = 3;
		}
	}
	spin_unlock_irqrestore(&hw->bufspec_lock, flags);
...
#endif
	dpb_print(DECODE_ID(hw), 0, "ammvdec_h264 mem-addr=%lx,buff_offset=%x,buf_start=%lx, dw %d\n",
		pdata->mem_start, hw->buf_offset, hw->cma_alloc_addr, hw->double_write_mode);

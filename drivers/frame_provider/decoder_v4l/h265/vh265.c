...
#define MMU_COPY_IDX(n)	(BUF_POOL_SIZE + n + 1)

#else
#define BMMU_MAX_BUFFERS (BUF_POOL_SIZE * 3 + 1)
#define VF_BUFFER_IDX(n)	(n)
#define BMMU_WORKSPACE_ID	(BUF_POOL_SIZE)
#define MV_BUFFER_IDX(n) (BUF_POOL_SIZE + 1 + n)
#define MMU_COPY_IDX(n)	(BUF_POOL_SIZE * 2 + n + 1)
#endif
...
static struct BuffInfo_s amvh265_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	...
	},
	{
		...
		.mpred_above		= {.buf_size = 0x4000}, //(pic_width/64)*16*16 byte = 0x4000
#ifdef MV_USE_FIXED_BUF
...
static int check_pic_decoded_lcu(struct hevc_state_s *hevc, struct PIC_s* cur_pic)
{
...
#if 0
static struct aml_buf *index_to_aml_buf(struct hevc_state_s *hevc, int index)
{
	return (struct aml_buf *)hevc->m_PIC[index]->cma_alloc_addr;
}
#endif
		...
		unsigned char flush_flag)
{
	...
	if (hevc->i_only & 0x4) {
		...
		if (pic_display && (pic_display->need_mmu_copy))
			pic_display = NULL;
			...
			if ((pic_display->error_mark &&
				...
				if (flush_flag == 2) {
					if (kfifo_peek(&hevc->pending_q, &vf)) {
						hevc_print_cont(hevc, H265_DEBUG_BUFMGR,
							"%s: pending_q len %d index 0x%x\n",
	...
	if (cur_pic->slice_type != 2) { /* P and B pic */
		...
		for (i = 0; i < cur_pic->RefNum_L0; i++) {
			...
			if (pic) {
				...
				if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
					hevc_print_cont(hevc, 0, "refid %x mc_canvas_u_v %x",
						i, pic->mc_canvas_u_v);
...
static int get_idle_pos(struct hevc_state_s *hevc)
{
	...
	for (i = 0; i < hevc->used_buf_num; ++i) {
		...
		if ((!(ctx->avbcd_work_mode) &&
			...
			(ctx->avbcd_work_mode && hevc->aml_buf &&
			(pic->am_buf == hevc->aml_buf)))
			break;
	}
		...
		bool check_dma)
{
	...
	u32 used_4k_num = hevc->used_4k_num;

	if (hevc->cur_pic == NULL)
		return 0;

	index = hevc->cur_pic->BUF_index;

	if ((hevc->cur_pic->need_mmu_copy) &&
		(hevc->cur_pic->used_4k_num == 0))
		return 0;
	...
	if (hevc->cur_pic->used_4k_num)
		used_4k_num = hevc->cur_pic->used_4k_num;
		...
		if ((pic->lcu_cnt < (hevc->lcu_x_num * hevc->lcu_y_num - 1))
			...
			if (afbc_buf != NULL) {
				...
				hevc_print(hevc, PRINT_FLAG_VDEC_DETAIL,
					...
					pic->POC, afbc_buf->POC, pic->lcu_cnt, pic->decoder_tile_cnt);
			}
		}
...
#endif
		int pic_height_cu =
			(hevc->pic_h + hevc->lcu_size - 1) >> hevc->lcu_size_log2;
		int pic_width_cu =
...
static int H265_alloc_mmu(struct hevc_state_s *hevc, struct PIC_s *new_pic,
	...
	hevc_print(hevc, H265_DEBUG_BUFMGR,
		...
		__func__, new_pic->index,
		cur_mmu_4k_number, aml_buf->fbc->index, aml_buf->planes[0].addr, ret);
	...
	while (p+4 <= sei_buf+size) {
		...
		if (p+payload_size <= sei_buf+size) {
			...
			case SEI_ContentLightLevel:
				...
				if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
					hevc_print_cont(hevc, 0,
						"content_light_level = %d, content_light_level = %d\n",
					hevc->content_light_level[0],
					hevc->content_light_level[1]);
				break;
	...
	u8 *hdr10p_data_buf, u32 data_size)
{
	...
	if (pic->sei_present_flag & SEI_HDR10PLUS_MASK) {
		...
		usd_rep.meta_data.records_in_que = kfifo_len(&ctx->dec_intf.aux_done) + 1; /* +1 : current one's */
...
static struct vframe_s *vh265_vf_get(void *op_arg)
{
	...
	if ((!ctx->avbcd_work_mode && kfifo_get(&hevc->display_q, &vf)) ||
		(ctx->avbcd_work_mode && kfifo_get(&hevc->avbc_display_q, &vf))) {
...
static void h265_avbc_done_cb(void *output)
{
	...
	hevc = (struct hevc_state_s *)vdec->private;
	...
	if (index_top != 0xff
		...
		if (hevc->m_PIC[index_top]->vf_ref > 0) {
			...
			hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,"index_top %d vf_ref %d output_ready %d\n",
			index_top, hevc->m_PIC[index_top]->vf_ref, hevc->m_PIC[index_top]->output_ready);
		}
	}
	...
	if (index_bot != 0xff
		...
		if (hevc->m_PIC[index_bot]->vf_ref > 0) {
			...
			hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,"index_bot %d vf_ref %d output_ready %d\n",
			index_bot, hevc->m_PIC[index_bot]->vf_ref, hevc->m_PIC[index_bot]->output_ready);
		}
	}
...
static void get_pair_fb(struct hevc_state_s *hevc, struct vframe_s *vf)
{
	...
	for (i = 0; i < 2; i ++) {
		if (hevc->pair_fb[i] == NULL) {
			...
			if (ctx->enable_di_post && !ctx->avbcd_work_mode)
...
static void put_vf_to_avbc_q(struct hevc_state_s *hevc, struct vframe_s *vf)
{
	if (kfifo_get(&hevc->display_q, &vf)) {
		kfifo_put(&hevc->avbc_display_q, (const struct vframe_s *)vf);
		hevc_print(hevc, H265_DEBUG_BUFMGR,
			"%s(vf %px type %d index 0x%x poc %d/%d) avbc_display_q %d ts %llu\n",
			__func__, vf, vf->type, vf->index,
			get_pic_poc(hevc, vf->index & 0xff),
			get_pic_poc(hevc, (vf->index >> 8) & 0xff),
			kfifo_len(&hevc->avbc_display_q),
			vf->timestamp);
	}
}
...
static void h265_post_avbcd_task(struct hevc_state_s *hevc)
{
	...
	mutex_lock(&hevc->post_mutex);

	if (!ctx->avbcd_work_mode)
		goto out;

	if (!kfifo_peek(&hevc->avbc_display_q, &vf))
		goto out;

	am_buf = aml_buf_get(&ctx->bm, BUF_USER_AVBCD, false);
	if (!am_buf) {
		hevc_print(hevc, PRINT_FLAG_VDEC_STATUS, "avbc no frame buffers!\n");
		goto out;
	}
	...
	if (hevc->bit_depth_luma == 8)
		...
		vf->canvas0_config[0].bit_depth = 1;
	}
	...
	vf->canvas0_config[0].block_mode = hevc->mem_map_mode;
	vf->canvas0_config[0].endian = 0;
	vf->canvas0_config[1].block_mode = hevc->mem_map_mode;
...
/*SUPPORT_10BIT*/
		...
		if (without_display_mode == 0) {
			if (v4l2_ctx->is_stream_off && ((!v4l2_ctx->avbcd_work_mode) ||
				(v4l2_ctx->avbcd_work_mode && hevc->first_pic_flag))) {
					...
					while (kfifo_len(&hevc->display_q)) {
						if (hevc->pair_fb[0] != NULL && hevc->pair_fb[1] != NULL) {
							...
							if (v4l2_ctx->avbcd_work_mode) {
								put_vf_to_avbc_q(hevc, vf);
								h265_post_avbcd_task(hevc);
							} else
			     ...
			     struct aml_vdec_ps_infos *ps)
{
	...
	if (ctx->avbcd_work_mode && hevc->double_write_mode != DM_AVBC_ONLY) {
		struct aml_vdec_cfg_infos cfg_info = { 0 };

		hevc->double_write_mode = DM_AVBC_ONLY;
		hevc_print(hevc, H265_DEBUG_DETAIL, "avbc mode double_write_mode %d\n", hevc->double_write_mode);
		vdec_v4l_get_cfg_infos(ctx, &cfg_info);
...
static int get_free_buf_count( struct hevc_state_s *hevc)
{
	...
	for (i = 0; i < hevc->used_buf_num; ++i) {
		...
		if ((pic->output_mark == 0) &&
			...
			(pic->output_ready == 0) &&
			(pic->vf_ref == 0) &&
			((!ctx->avbcd_work_mode && !pic->cma_alloc_addr) ||
			ctx->avbcd_work_mode)) {
			free_slot++;
	...
	if (ctx->avbcd_work_mode) {
		...
		if (!hevc->aml_buf) {
			if (!pic->buffer_attached) {
				pic->am_buf = aml_buf_alloc_avbcd_buf(&ctx->bm);
				pic->am_buf->task->attach(pic->am_buf->task, &task_dec_ops,  hw_to_vdec(hevc));
				pic->am_buf->state = FB_ST_DECODER;
				pic->buffer_attached = true;
			}
			hevc->aml_buf = pic->am_buf;
		} else
			hevc_print(hevc, H265_DEBUG_BUFMGR,
			"%s already got buf(%px, %d)!\n",
				__func__, pic->am_buf, pic->am_buf->index);
...
/*#endif*/
			...
			if (!v4l_res_change(hevc, &hevc->param)) {
				if (ctx->param_sets_from_ucode && !hevc->v4l_params_parsed) {
					...
					hevc->used_buf_num = pic.dpb_frames +
						pic.dpb_margin;
					if (ctx->avbcd_work_mode)
						hevc->used_buf_num = pic.dpb_frames + 1;
	...
	if (efficiency_mode && dec_status == HEVC_SLICE_SEGMENT_DONE) {
		if ((hevc->wait_buf == 0 && hevc->param.p.slice_segment_address == 0) &&
			(hevc->m_nalUnitType != NAL_UNIT_CODED_SLICE_IDR &&
			hevc->m_nalUnitType != NAL_UNIT_CODED_SLICE_IDR_N_LP)) {
			output_pic_display(hevc, 0, false);
		}
...
static irqreturn_t vh265_isr(int irq, void *data)
{
	...
	if (!hevc->m_ins_flag) {
		if (dec_status == HEVC_OVER_DECODE) {
			hevc->over_decode = 1;
			hevc_print(hevc, 0, "isr: over decode\n");
			WRITE_VREG(HEVC_DEC_STATUS_REG, 0);
			return IRQ_HANDLED;
		}
	}
 ...
 */
	...
	INIT_KFIFO(hevc->avbc_display_q);
...
static bool is_available_buffer(struct hevc_state_s *hevc)
{
	...
	if (!free_slot) {
		...
		if (pic == NULL)
			continue;
		hevc_print(hevc, H265_DEBUG_BUFMGR,
				"%s buf idx %d output_mark: %d dma addr: 0x%lx vf_ref %d output_ready %d ref %d\n",
				__func__, i,pic->output_mark,
				pic->cma_alloc_addr,
				pic->vf_ref,
				pic->output_ready,
...
int find_near_pic_index(struct hevc_state_s *hevc, struct PIC_s* cur_pic)
{
	...
	if (index != -1) {
		for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
			...
			if ((afbc_buf->used) && (afbc_buf->POC != INVALID_POC))
				hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
					"fb %p, used %d, POC %d\n",
					afbc_buf->fb, afbc_buf->used, afbc_buf->POC);
		}
	}
...
void get_mmu_copy_map(struct hevc_state_s* hevc, struct PIC_s* pic, u32 page_num)
{
	...
	for (i = 0; i < page_num - 1; i++) {
		...
		if (map[i] > 0x100000) {
			if (mmu_copy->mmu_copy_buf_start && hevc->bmmu_box) {
				decoder_bmmu_box_free_idx(hevc->bmmu_box, MMU_COPY_IDX(pic->index));
				mmu_copy->mmu_copy_buf_start = 0;
			}
			pic->need_mmu_copy = 0;
			hevc_print(hevc, 0,
...
static void error_handle_mmu_copy(struct hevc_state_s *hevc, struct PIC_s* pic)
{
...
static int ammvdec_h265_probe(struct platform_device *pdev)
{
	...
	if (!is_support_triple_write()) {
		if ((hevc->triple_write_mode) || (triple_write_mode)) {
			...
			hevc_print(hevc, 0, "unsupport triple write mode, force disable\n");
		}
	}
	...
	if (!is_support_p010_mode()) {
		...
		if (is_tw_p010(hevc)) {
			...
			hevc->triple_write_mode &= ~(1 <<16);
			hevc_print(hevc, 0, "unsupport tw p010 mode, force disable\n");
		}
	}
...
static int __init amvdec_h265_driver_init_module(void)
{
	...
	vcodec_profile_register_v2("H.265-V4L", VFORMAT_HEVC, 1);

...
#define MULTI_DRIVER_NAME "ammvdec_h265_fb_v4l"
#endif
#define DRIVER_NAME "amvdec_h265_fb_v4l"
...
static int h265_reset_frame_buffer(struct hevc_state_s *hevc);
...
static unsigned int decode_timeout_back_fast_check = 1;
static unsigned int decode_timeout_back_fast_check_threshold = 3;
#endif
...
#define	DEFAULT_REF_PIC_ID			(BMMU_IMEM_ID + 1)
...
#define BMMU_MAX_BUFFERS (BUF_POOL_SIZE * 2 + 1 + FB_LOOP_BUF_COUNT)
#define VF_BUFFER_IDX(n)	(FB_LOOP_BUF_COUNT + n)
#define BMMU_WORKSPACE_ID	(FB_LOOP_BUF_COUNT + BUF_POOL_SIZE)
#define MMU_COPY_IDX(n)	(FB_LOOP_BUF_COUNT + BUF_POOL_SIZE + 1 + n)
#else
#define BMMU_MAX_BUFFERS (BUF_POOL_SIZE * 3 + 1 + FB_LOOP_BUF_COUNT)
#define VF_BUFFER_IDX(n)	(FB_LOOP_BUF_COUNT + n)
#define BMMU_WORKSPACE_ID	(FB_LOOP_BUF_COUNT + BUF_POOL_SIZE)
#define MV_BUFFER_IDX(n) (FB_LOOP_BUF_COUNT + BUF_POOL_SIZE + 1 + n)
#define MMU_COPY_IDX(n)	(FB_LOOP_BUF_COUNT + BUF_POOL_SIZE * 2 + 1 + n)
#endif
...
/*
 ...
 2: Use the closest frame to the current frame as the reference frame.
 3: Use the current frame as a reference.
 */
static u32 error_handle_mode = 2;
 ...
 *	1: no support rdma
 *	0: support rdma
 *bit [23] single core ucode check multi slice info
...
static struct BuffInfo_s amvh265_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	...
	},
	{
		...
		.dblk_para		= {.buf_size = 0x18100}, // dblk parameter addr_offset_lft(8k?32:16)*4096 + (ctu_numb_y+1)*256 = 16*4096+(2048/16+1)*256=0x18100
		.dblk_data		= {.buf_size = 0x80800}, // dblk data for left/top addr_offset_lft(64)*4096 + (ctu_numb_y+1)*2048 = 64*4096+(2048/16+1)*2048= 0x80800
		.dblk_data2		= {.buf_size = 0x40800}, // dblk data for adapter addr_offset_lft(0)*4096 + (ctu_numb_y+1)*2048 = 0*4096+(2048/16+1)*2048= 0x40800
		.mmu_vbh		= {.buf_size = VBH_BUF_SIZE_1080P}, //2*16*2304/4, 4K
		.mpred_above		= {.buf_size = 0x2000}, //(pic_width/64)*16*16 byte = 2048/64*16*16=0x2000
#ifdef MV_USE_FIXED_BUF
		.mpred_mv		= {.buf_size = MPRED_MV_BUF_SIZE * MAX_REF_PIC_NUM}, //s1080p, 0x40000 per buffer */
#endif
		.rpm			= {.buf_size = RPM_BUF_SIZE},
		.lmem			= {.buf_size = 0x500 * 2},
	},
	{
		.max_width		= 4096,
		.max_height		= 2048,
		...
		.swap_buf2		= {.buf_size = 0x800},
		.scalelut		= {.buf_size = 0x8000}, //support up to 32 SCALELUT 1024x32 = 32Kbytes = (0x8000)
		.dblk_para		= {.buf_size = 0x20100}, // dblk parameter addr_offset_lft(8k?32:16)*4096 + (ctu_numb_y+1)*256 = 16*4096+(4096/16+1)*256=0x20100
		.dblk_data		= {.buf_size = 0xc0800}, // dblk data for left/top addr_offset_lft(64)*4096 + (ctu_numb_y+1)*2048 = 64*4096+(4096/16+1)*2048= 0xc0800
		.dblk_data2		= {.buf_size = 0x80800}, // dblk data for adapter addr_offset_lft(0)*4096 + (ctu_numb_y+1)*2048 = 0*4096+(4096/16+1)*2048= 0x80800
		.mmu_vbh		= {.buf_size = VBH_BUF_SIZE_4K}, /*2*16*2304/4, 4K*/
		.mpred_above		= {.buf_size = 0x4000},
#ifdef MV_USE_FIXED_BUF
		.mpred_mv		= {.buf_size = MPRED_4K_MV_BUF_SIZE * MAX_REF_PIC_NUM}, // 4096x2304 , 0x120000 per buffer
#endif
		.rpm			= {.buf_size = RPM_BUF_SIZE},
		.lmem			= {.buf_size = 0x500 * 2},
	},
	{
		...
		.swap_buf		= {.buf_size = 0x800}, // 256cyclex64bit = 2K bytes 0x800 (only 144 cycles valid)
		.swap_buf2		= {.buf_size = 0x800},
		.scalelut		= {.buf_size = 0x8000}, //0x8000*2, support up to 32 SCALELUT 1024x32 = 32Kbytes (0x8000)
		.dblk_para		= {.buf_size = 0x40100}, // dblk parameter addr_offset_lft(8k?32:16)*4096 + (ctu_numb_y+1)*256 = 32*4096+(8192/16+1)*256=0x40100
		.dblk_data		= {.buf_size = 0x140800}, // dblk data for left/top addr_offset_lft(64)*4096 + (ctu_numb_y+1)*2048 = 64*4096+(8192/16+1)*2048= 0x140800
		.dblk_data2		= {.buf_size = 0x100800}, // dblk data for adapter addr_offset_lft(0)*4096 + (ctu_numb_y+1)*2048 = 0*4096+(8192/16+1)*2048= 0x100800
		.mmu_vbh		= {.buf_size = VBH_BUF_SIZE_8K}, //2*16*2304/4, 4K
		.mpred_above		= {.buf_size = 0x8000},
#ifdef MV_USE_FIXED_BUF
		.mpred_mv		= {.buf_size = MPRED_8K_MV_BUF_SIZE * MAX_REF_PIC_NUM}, //4k2k , 0x120000 per buffer
#endif
		.rpm			= {.buf_size = RPM_BUF_SIZE},
		.lmem			= {.buf_size = 0x500 * 2},
	}
...
static void error_handle_mmu_copy(struct hevc_state_s *hevc, PIC_t* pic);
...
struct hevc_state_s {
	...
	int back_timer_check_count;
	int back_timer_check_value;
	u32 data_size;
	u32 data_offset;
	u32 data_invalid;
	u32 consume_byte;
	u32 multi_frame_flag;
	int slice_count;
...
void pic_backend_ref_operation(struct hevc_state_s *hevc, bool add_flag)
{
	PIC_t *pic = NULL;
	int i = 0;
	...
	for (i = 0; (i < MAX_REF_PIC_NUM) && (pic->ref_pic[i] != NULL); i++) {
		if (add_flag) {
			...
			if (pic->ref_pic[i]->backend_ref < 0) {
				...
				hevc_print(hevc, PRINT_FLAG_VDEC_DETAIL, "%s:ref_pic(%px) backend_ref error\n",
...
int find_near_pic_index(struct hevc_state_s *hevc, PIC_t* cur_pic);
void clean_decoder_info(struct hevc_state_s *hevc, struct PIC_s* cur_pic);

static int check_pic_decoded_lcu(struct hevc_state_s *hevc, PIC_t* cur_pic)
{
	int lcu_total = hevc->lcu_x_num * hevc->lcu_y_num - 1;
	int decoder_lcu = cur_pic->lcu_cnt;

	if ((cur_pic->tile_cnt > 1) && (decoder_lcu < lcu_total)
		&& ((cur_pic->decoder_tile_cnt + 1) < cur_pic->tile_cnt)) {
		int pic_width_cu = (hevc->pic_w + hevc->lcu_size - 1) >> hevc->lcu_size_log2;
		int lcu_x = (decoder_lcu + 1) % pic_width_cu;
		int lcu_y = (decoder_lcu + 1) / pic_width_cu;
		int tile_i = cur_pic->decoder_tile_cnt % hevc->num_tile_col;
		int tile_j = cur_pic->decoder_tile_cnt / hevc->num_tile_col;
		int tile_x = lcu_x - hevc->m_tile[tile_j][tile_i].start_cu_x;
		int tile_y = lcu_y - hevc->m_tile[tile_j][tile_i].start_cu_y;

		if (cur_pic->decoder_tile_cnt >= 1)
			decoder_lcu = hevc->tile_lcu[cur_pic->decoder_tile_cnt - 1] - 1
				+ hevc->m_tile[tile_j][tile_i].width * tile_y + tile_x;
		else
			decoder_lcu = hevc->m_tile[tile_j][tile_i].width * tile_y + tile_x;

		hevc_print(hevc, H265_DEBUG_BUFMGR,
			"x %d y %d decoder_lcu(%d), lcu_cnt %d decoder_tile_cnt %d\n",
			tile_x, tile_y, decoder_lcu, cur_pic->lcu_cnt, cur_pic->decoder_tile_cnt);
	}

	if (decoder_lcu < (lcu_total * (100 - hevc->lcu_percentage_threshold) / 100)) {
		hevc_print(hevc, H265_DEBUG_BUFMGR,
			"%s:poc %d decoded_lcu %d drop flag is true\n",
			__func__, cur_pic->POC, decoder_lcu);
		return 1;
	}
...
static int front_decpic_done_update(struct hevc_state_s *hevc, uint8_t reset_flag)
{
	...
	cur_pic->lcu_cnt = READ_VREG(HEVC_PARSER_LCU_START) & 0xffffff;
	cur_pic->decoder_tile_cnt = READ_VREG(DECODER_PICTURE_INFO) & 0xff;

	if (hevc->lcu_percentage_threshold) {
		cur_pic->drop_flag = check_pic_decoded_lcu(hevc, cur_pic);
	...
	if ((hevc->PB_skip_mode == 0)
		...
		if ((cur_pic->lcu_cnt < (hevc->lcu_x_num * hevc->lcu_y_num - 1))
			&& ((cur_pic->tile_cnt == 1) ||
			(cur_pic->decoder_tile_cnt < cur_pic->tile_cnt))) {
			struct afbc_buf* afbc_buf = NULL;
			int index = find_near_pic_index(hevc, cur_pic);

			if (index != -1)
				afbc_buf = &hevc->afbc_buf_table[index];

			if (afbc_buf != NULL) {
				struct aml_buf *aml_buf = (struct aml_buf *)afbc_buf->fb;

				cur_pic->need_mmu_copy = 1;
				cur_pic->mmu_copy_header_adr = aml_buf->fbc->haddr;
				cur_pic->decoder_tile_cnt =
					(READ_VREG(HEVC_DECODE_INFO) >> 16) & 0xff;

				hevc_print(hevc, PRINT_FLAG_VDEC_DETAIL,
					"CUR_POC(%d) mmu copy POC(%d), lcu_cnt %d decoder_tile_cnt %d\n",
					cur_pic->POC, afbc_buf->POC, cur_pic->lcu_cnt, cur_pic->decoder_tile_cnt);
			}
		}

		clean_decoder_info(hevc, cur_pic);
	}
...
static enum ResResult is_csd_valid(struct hevc_state_s *hevc, int w, int h) {
	...
	if (over_size == RES_RET_OVERSIZE) {
		...
		if (hevc->old_csd_info_check_count >= 3)
...
static __inline__ bool is_tw_p010(struct hevc_state_s *hevc)
{
	unsigned int out, tw;
...
static bool is_p010_mode(struct hevc_state_s *hevc)
{
	return is_dw_p010(hevc) || is_tw_p010(hevc);
}
...
{
	memcpy(vf, vh265_vf_get(vdec_ctx), sizeof(struct vframe_s));
}
...
static int v4l_alloc_buf(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (aml_buf->num_planes == 1) {
		...
		if (tw_mode) {
			hevc->m_BUF[i].start_adr_tw = aml_buf->planes_tw[0].addr;
			hevc->m_BUF[i].luma_size_tw = aml_buf->planes_tw[0].offset;
			hevc->m_BUF[i].size_tw = aml_buf->planes_tw[0].length;
			aml_buf->planes_tw[0].bytes_used = aml_buf->planes_tw[0].length;
			pic->tw_y_adr = hevc->m_BUF[i].start_adr_tw;
			pic->tw_u_v_adr = pic->tw_y_adr + hevc->m_BUF[i].luma_size_tw;
		...
		if (tw_mode) {
			hevc->m_BUF[i].start_adr_tw = aml_buf->planes_tw[0].addr;
			hevc->m_BUF[i].luma_size_tw = aml_buf->planes_tw[0].length;
			hevc->m_BUF[i].chroma_addr_tw = aml_buf->planes_tw[1].addr;
			hevc->m_BUF[i].chroma_size_tw = aml_buf->planes_tw[1].length;
			hevc->m_BUF[i].size_tw = aml_buf->planes_tw[0].length + aml_buf->planes_tw[1].length;
			aml_buf->planes_tw[0].bytes_used = aml_buf->planes_tw[0].length;
			aml_buf->planes_tw[1].bytes_used = aml_buf->planes_tw[1].length;
			pic->tw_y_adr = hevc->m_BUF[i].start_adr_tw;
			pic->tw_u_v_adr = hevc->m_BUF[i].chroma_addr_tw;
	...
	if (!vdec_secure(hw_to_vdec(hevc)) && (pic->dw_y_adr)
		&& (pic->dw_u_v_adr)
		&& (hevc->PB_skip_mode == 0)
		&& (dw_mode == 0x10)) {
		...
		if (aml_buf->num_planes == 1) {
			if (mem_start_virt_y) {
				memset(mem_start_virt_y, 0x80, pic->luma_size + pic->chroma_size);
				codec_mm_dma_flush(mem_start_virt_y,
					pic->luma_size + pic->chroma_size, DMA_TO_DEVICE);
			} else {
				codec_mm_memset(pic->dw_y_adr,
					0x80, pic->luma_size + pic->chroma_size);
			}
		} else if (aml_buf->num_planes == 2) {
			void *mem_start_virt_u_v = codec_mm_phys_to_virt(pic->dw_u_v_adr);

			if (mem_start_virt_y) {
				memset(mem_start_virt_y, 0x80, pic->luma_size);
				codec_mm_dma_flush(mem_start_virt_y,
					pic->luma_size, DMA_TO_DEVICE);
			} else {
				codec_mm_memset(pic->dw_y_adr,
					0x80, pic->luma_size);
			}

			if (mem_start_virt_u_v) {
				memset(mem_start_virt_u_v, 0x80, pic->chroma_size);
				codec_mm_dma_flush(mem_start_virt_u_v,
					pic->chroma_size, DMA_TO_DEVICE);
			} else {
				codec_mm_memset(pic->dw_u_v_adr,
					0x80, pic->chroma_size);
			}
		}
	}
...
static int v4l_config_pic(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	hevc_print(hevc, H265_DEBUG_BUFMGR,
		"%s: mc_canvas_y %d, mc_canvas_u_v %d mc_y_adr %px, mc_u_v_adr %px, header_adr %p\n",
		__func__, pic->mc_canvas_y, pic->mc_canvas_u_v, pic->mc_y_adr, pic->mc_u_v_adr, pic->header_adr);
...
#endif
		PR_FILL("output_ready:%d, dma addr %lx mv_wr_start %x vf_ref %d need_mmu_copy %d",
				pic->output_ready, pic->cma_alloc_addr, pic->mpred_mv_wr_start_addr,
				pic->vf_ref, pic->need_mmu_copy);
		PR_INFO(hevc->index);
	}
}
...
static int config_mc_buffer(struct hevc_state_s *hevc, struct PIC_s *cur_pic)
{
	...
	if (cur_pic->slice_type != 2) {	/* P and B pic */
		...
		for (i = 0; i < cur_pic->RefNum_L0; i++) {
			...
			if (pic) {
				...
				if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
					hevc_print_cont(hevc, 0, "refid %x mc_canvas_u_v %d",
						i, pic->mc_canvas_u_v);
					hevc_print_cont(hevc, 0, " mc_canvas_y %d\n",
						pic->mc_canvas_y);
				}
			} else {
				cur_pic->error_mark = 1;
				if (hevc->PB_skip_mode == 0) {
					if ((hevc->error_handle_mode == 1) && (hevc->ref_pic.dw_y_adr != 0)) {
						...
						hevc_print(hevc, H265_DEBUG_BUFMGR,
							"%s, set Default Ref to Ref\n", __func__);
						hevc_print(hevc, H265_DEBUG_BUFMGR,
							"refid %x mc_canvas_u_v %d mc_canvas_y %d\n",
							i, hevc->ref_pic.mc_canvas_u_v, hevc->ref_pic.mc_canvas_y);
					}
				}
			}

			if (pic == NULL || pic->error_mark) {
				hevc_print(hevc, H265_DEBUG_BUFMGR, "Error %s, %dth poc (%d) %s",
	...
	if (cur_pic->slice_type == 0) {	/* B pic */
		hevc_print(hevc, H265_DEBUG_BUFMGR, "config_mc_buffer RefNum_L1\n");
		...
		for (i = 0; i < cur_pic->RefNum_L1; i++) {
			...
			if (pic) {
				...
				if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
					hevc_print_cont(hevc, 0,
						"refid %x mc_canvas_u_v %d", i, hevc->ref_pic.mc_canvas_u_v);
					hevc_print_cont(hevc, 0,
						" mc_canvas_y %d\n", hevc->ref_pic.mc_canvas_y);
				}
			} else {
				cur_pic->error_mark = 1;
				if (hevc->PB_skip_mode == 0) {
...
static int alloc_default_ref_buffer(struct hevc_state_s *hevc)
{
	struct PIC_s *pic = hevc->cur_pic;
	int dw_mode = get_double_write_mode(hevc);
	int y_buf_size = pic->luma_size;
	int buf_size = pic->chroma_size + y_buf_size;

	if ((hevc->ref_pic.dw_y_adr == 0) && (buf_size != 0)) {
		int ret = 0;
		ulong addr = 0;

		if ((buf_size > 0) && (dw_mode == 16)) {
			ret = decoder_bmmu_box_alloc_buf_phy(hevc->bmmu_box,
				DEFAULT_REF_PIC_ID, buf_size, DRIVER_NAME, &addr);
			if (ret < 0) {
				hevc->ref_pic.dw_y_adr = 0;
				hevc_print(hevc, PRINT_FLAG_ERROR,
					"%s, size: %d, no mem fatal err\n",
					__func__, buf_size);
				return 0;
			} else {
				hevc->ref_pic.dw_y_adr = addr;
				hevc->ref_pic.dw_u_v_adr = hevc->ref_pic.dw_y_adr + y_buf_size;
				hevc->ref_pic.mc_y_adr = hevc->ref_pic.dw_y_adr;
				hevc->ref_pic.mc_u_v_adr = hevc->ref_pic.dw_u_v_adr;

				if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
					hevc_print(hevc, 0,
						"Error Ref Buffer: dw_y_adr %p size:%d dw_uv_adr %p size\n",
						hevc->ref_pic.dw_y_adr, y_buf_size,
						hevc->ref_pic.dw_u_v_adr, buf_size - y_buf_size);
				}

				/*flush the buffer make sure no cache dirty*/
				if (!vdec_secure(hw_to_vdec(hevc))) {
					void *mem_start_virt = codec_mm_phys_to_virt(hevc->ref_pic.dw_y_adr);

					if (mem_start_virt) {
						memset(mem_start_virt, 0x80, buf_size);
						codec_mm_dma_flush(mem_start_virt,
							buf_size, DMA_TO_DEVICE);
					} else {
						mem_start_virt = codec_mm_vmap(
							hevc->ref_pic.dw_y_adr, buf_size);
						if (mem_start_virt) {
							memset(mem_start_virt, 0x80, buf_size);
							codec_mm_dma_flush(mem_start_virt,
								buf_size, DMA_TO_DEVICE);
							...
							/*not virt for tvp playing, may need clear on ucode.*/
							hevc_print(hevc, 0, "ref %s mem_start_virt failed\n", __func__);
						}
					}
				}
			}
	...
	if (hevc->ref_pic.dw_y_adr != 0) {
		...
		for (i = 0; i < 16; i++) {
			...
			if (hevc->m_PIC[i]->referenced == 0) {
				hevc->ref_pic.mc_canvas_y = i * 2;
				hevc->ref_pic.mc_canvas_u_v = i * 2 + 1;

				hevc_print(hevc, H265_DEBUG_BUFMGR,
					"Default Ref Buffer: mc_canvas_u_v:%d mc_canvas_y:%d\n",
					hevc->ref_pic.mc_canvas_u_v, hevc->ref_pic.mc_canvas_y);
				break;
			}
		}
	}
...
static void default_ref_hw_config(struct hevc_state_s *hevc)
{
	int dw_mode = get_double_write_mode(hevc);

	if (hevc->ref_pic.dw_y_adr != 0) {
		if ((hevc->front_back_mode == 1) || (hevc->front_back_mode == 3)) {
			WRITE_BACK_32(hevc, HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR,
				(0x1 << 1) | (0x1 << 2) | (hevc->ref_pic.mc_canvas_y << 8));

			if (hevc->mmu_enable && ((dw_mode & 0x10) == 0)) {
/*
				WRITE_BACK_32(hevc, HEVCD_MPP_ANC2AXI_TBL_DATA,
					hevc->ref_pic.header_adr >> 5);
*/
			} else {
				WRITE_BACK_32(hevc, HEVCD_MPP_ANC2AXI_TBL_DATA,
					hevc->ref_pic.mc_y_adr >> 5);
			}

			if (dw_mode & 0x10) {
				WRITE_BACK_32(hevc, HEVCD_MPP_ANC2AXI_TBL_DATA,
					hevc->ref_pic.mc_u_v_adr >> 5);
			}
...
static int ref_pic_error_handle(struct hevc_state_s *hevc)
{
	struct PIC_s *pic = hevc->cur_pic;
	int i = 0;
	int index = 0;
	struct PIC_s *tmp_pic = NULL;
	int l0_num = 0;
	int l1_num = 0;
	...
	if ((hevc->PB_skip_mode != 0) || (pic->slice_type == I_SLICE))
		return 0;

	if (hevc->error_handle_mode == 2) {
		for (i = 0; i < MAX_REF_PIC_NUM; i++) {
			tmp_pic = hevc->m_PIC[i];
			if ((tmp_pic == NULL)
				|| (tmp_pic->index == -1)
				|| (tmp_pic->BUF_index == -1))
				continue;

			if ((tmp_pic->POC < pic->POC)
				&& ((pic->POC - tmp_pic->POC) < f_diff)) {
	...
	for (i = 0; i < pic->RefNum_L0; i++) {
		tmp_pic = get_ref_pic_by_POC(hevc, pic->m_aiRefPOCList0[pic->slice_idx][i]);
		if (tmp_pic == NULL) {
			pic->error_mark = 1;
			if (hevc->error_handle_mode == 1) {
				l0_num++;
				hevc_print(hevc, H265_DEBUG_BUFMGR, "ref0 POC(%d) is not present\n",
					pic->m_aiRefPOCList0[pic->slice_idx][i]);
			} else if (hevc->error_handle_mode == 2) {
				if (f_diff != 0xffffffff) {
					index = f_index;
				} else if (b_diff != 0xffffffff) {
					index = b_index;
				} else {
					index = pic->index;
				}

				hevc_print(hevc, H265_DEBUG_BUFMGR, "ref0 POC(%d) is not exist, replace it with POC(%d)\n",
					pic->m_aiRefPOCList0[pic->slice_idx][i], hevc->m_PIC[index]->POC);
				pic->m_aiRefPOCList0[pic->slice_idx][i] = hevc->m_PIC[index]->POC;
			} else if (hevc->error_handle_mode == 3) {
				index = pic->index;

				hevc_print(hevc, H265_DEBUG_BUFMGR, "ref0 POC(%d) is not exist, replace it cur_pic's with POC(%d)\n",
					pic->m_aiRefPOCList0[pic->slice_idx][i], hevc->m_PIC[index]->POC);
				pic->m_aiRefPOCList0[pic->slice_idx][i] = hevc->m_PIC[index]->POC;
			}
		}
	...
	for (i = 0; i < pic->RefNum_L1; i++) {
		tmp_pic = get_ref_pic_by_POC(hevc, pic->m_aiRefPOCList1[pic->slice_idx][i]);
		if (tmp_pic == NULL) {
			pic->error_mark = 1;
			if (hevc->error_handle_mode == 1) {
				l1_num++;
				hevc_print(hevc, H265_DEBUG_BUFMGR, "ref1 POC(%d) is not present\n",
					pic->m_aiRefPOCList1[pic->slice_idx][i]);
			} else if (hevc->error_handle_mode == 2) {
				if (b_diff != 0xffffffff) {
					index = b_index;
				} else if (f_diff != 0xffffffff) {
					index = f_index;
				} else {
					index = pic->index;
				}

				hevc_print(hevc, H265_DEBUG_BUFMGR, "ref1 POC(%d) is not exist, replace it with POC(%d)\n",
					pic->m_aiRefPOCList1[pic->slice_idx][i], hevc->m_PIC[index]->POC);
				pic->m_aiRefPOCList1[pic->slice_idx][i] = hevc->m_PIC[index]->POC;
			} else if (hevc->error_handle_mode == 3) {
				index = pic->index;

				hevc_print(hevc, H265_DEBUG_BUFMGR, "ref1 POC(%d) is not exist, replace it with cur_poc's POC(%d)\n",
					pic->m_aiRefPOCList1[pic->slice_idx][i], hevc->m_PIC[index]->POC);
				pic->m_aiRefPOCList1[pic->slice_idx][i] = hevc->m_PIC[index]->POC;
			}
		}
	...
	if (((l1_num != 0) || (l0_num != 0)) && (hevc->error_handle_mode == 1)) {
		alloc_default_ref_buffer(hevc);
		default_ref_hw_config(hevc);
	}

	hevc_print(hevc, H265_DEBUG_BUFMGR, "ref0 POC:");
	for (i = 0; i < pic->RefNum_L0; i++) {
		if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
			PR_FILL("%d ", pic->m_aiRefPOCList0[pic->slice_idx][i]);
		}
	}
	if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR)
		PR_INFO(hevc->index);
	...
	if (pic->slice_type == B_SLICE) {
		...
		for (i = 0; i < pic->RefNum_L1; i++) {
			if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
				PR_FILL("%d ", pic->m_aiRefPOCList1[pic->slice_idx][i]);
			}
		}
		if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR)
			PR_INFO(hevc->index);
		...
		union param_u *params)
{
		...
		for (i = 0; i < hevc->num_tile_row; i++) {
			...
			for (j = 0; j < hevc->num_tile_col; j++) {
				if (params->p.tiles_enabled_flag & 2) { /* uniform flag */
					hevc->m_tile[i][j].width = (j + 1) * pic_width_cu / hevc->num_tile_col
						- j * pic_width_cu / hevc->num_tile_col;
					hevc->m_tile[i][j].height = (i + 1) * pic_height_cu / hevc->num_tile_row
						- i * pic_height_cu / hevc->num_tile_row;
				} else {
				...
				hevc->m_tile[i][j].sao_vb_start_addr =
					hevc->work_space_buf->sao_vb.buf_start + j * sao_vb_size;
				hevc->m_tile[i][j].sao_abv_start_addr =
					hevc->work_space_buf->sao_abv.buf_start + i * sao_abv_size;
	...
	for (i = 0; i < hevc->num_tile_row; i++) {
		for (j = 0; j < hevc->num_tile_col; j++) {
			if (k == 0)
				hevc->tile_lcu[k] = hevc->m_tile[i][j].width * hevc->m_tile[i][j].height;
			else
				hevc->tile_lcu[k] = hevc->tile_lcu[k - 1] +
					hevc->m_tile[i][j].width * hevc->m_tile[i][j].height;
			if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
				hevc_print(hevc, 0, "%s hevc->tile_lcu[%d] = %d\n",
					__func__, k, hevc->tile_lcu[k]);
			}
			k++;
		}
	}
...
static void config_mpred_hw(struct hevc_state_s *hevc)
{
	...
	if (col_pic == NULL) {
		mpred_mv_rd_start_addr = cur_pic->mpred_mv_wr_start_addr;
		col_isIntra = 1;
	} else
	...
	if (col_pic == NULL)
		mpred_mv_rd_end_addr = mpred_mv_rd_start_addr + cur_pic->mv_size;
	else
	...
	if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR) {
		hevc_print(hevc, 0,
			"cur pic index %d, col_isIntra %d\n", cur_pic->index, col_isIntra);
	}
...
/*SUPPORT_10BIT*/
	if (dw_mode & 0x10) {
		...
		data32 = READ_VREG(HEVC_SAO_CTRL5);
		data32 &= (~(0xff << 16));
		if ((dw_mode == 8) || (dw_mode == 9)) {
			data32 |= (0xff << 16);
			WRITE_VREG(HEVC_SAO_CTRL5, data32);
			WRITE_VREG(HEVC_SAO_CTRL26, 0xf);
		} else {
			if (dw_mode == 2 || dw_mode == 3)
				data32 |= (0xff<<16);
	...
	if (tw_mode) {
		WRITE_VREG(HEVC_SAO_Y_LENGTH3, cur_pic->luma_size_tw);
	...
	*/
	...
	if (tw_mode) {
		...
		if (debug & H265_DEBUG_REG) {
			hevc_print(hevc, 0, "triple write HEVC_SAO_CTRL3 %x, HEVC_SAO_CTRL31 %x, HEVC_SAO_CTRL32 %x\n",
			READ_VREG(HEVC_SAO_CTRL3), READ_VREG(HEVC_SAO_CTRL31), READ_VREG(HEVC_SAO_CTRL32));
		}
	}
		...
		union param_u *rpm_param)
{
	...
	new_pic->triple_write_mode = get_triple_write_mode(hevc);

	if (new_pic->double_write_mode || new_pic->triple_write_mode)
	...
	new_pic->need_mmu_copy = 0;
	new_pic->decoder_tile_cnt = 0;
	new_pic->lcu_cnt = 0;
	new_pic->used_4k_num = 0;
	new_pic->used_4k_num1 = 0;
#ifdef NEW_FRONT_BACK_CODE
	if (hevc->front_back_mode) {
...
static void vh265_report_err_timestamp_for_decoded_frames(struct aml_vcodec_ctx *ctx,
	struct PIC_s *pic)
{
	u64 timestamp_bak;
	timestamp_bak = ctx->current_timestamp;
	ctx->current_timestamp = pic->timestamp;
...
static int recycle_mmu_buf_tail(struct hevc_state_s *hevc,
		bool check_dma, struct PIC_s *pic)
{
	int index = 0;
	struct aml_buf *aml_buf;
	struct aml_vcodec_ctx *ctx = (struct aml_vcodec_ctx *)(hevc->v4l2_ctx);
	unsigned used_4k_num = READ_VREG(HEVC_SAO_MMU_STATUS) >> 16;
	unsigned used_4k_num1 = 0;

	if (pic == NULL)
		return 0;

	index = pic->BUF_index;
	if ((pic->need_mmu_copy) && (pic->used_4k_num == 0))
		return 0;
	...
	else if (hevc->front_back_mode == 1)
	...
	hevc_print(hevc,
			H265_DEBUG_BUFMGR,
			"%s pic index %d scatter_alloc %d page_start used_4k_num %d used_4k_num1 %d fb %px poc %d\n",
...
static void v4l_hevc_collect_stream_info(struct vdec_s *vdec,
	struct hevc_state_s *hevc)
{
	struct aml_vcodec_ctx *ctx = hevc->v4l2_ctx;
	...
	str_info->profile_idc = hevc->param.p.profile_etc;
	str_info->filed_flag = is_interlace(hevc);
	str_info->frame_height = hevc->frame_height;
	str_info->frame_width = hevc->frame_width;
	str_info->crop_top = hevc->param.p.conf_win_top_offset;
	str_info->crop_bottom = hevc->param.p.conf_win_bottom_offset;
	str_info->crop_left= hevc->param.p.conf_win_left_offset;
	str_info->crop_right = hevc->param.p.conf_win_right_offset;
			...
			if (hevc->mmu_enable
				...
				if (!hevc->m_ins_flag) {
					...
					if ((!is_skip_decoding(hevc, pic)) &&
						...
						ATRACE_COUNTER(hevc->trace.decode_header_memory_time_name, TRACE_HEADER_MEMORY_START);
						recycle_mmu_buf_tail(hevc, true, hevc->cur_pic);
						ATRACE_COUNTER(hevc->trace.decode_header_memory_time_name, TRACE_HEADER_MEMORY_END);
					}
				}
			}
	...
	int decoded_lcu)
{
	...
	if ((hevc->PB_skip_mode == 0)
		...
		if ((pic->lcu_cnt < (hevc->lcu_x_num * hevc->lcu_y_num - 1))
			&& ((pic->tile_cnt == 1) ||
			(pic->decoder_tile_cnt < pic->tile_cnt))) {
			struct afbc_buf* afbc_buf = NULL;
			int index = find_near_pic_index(hevc, pic);
			...
			if (afbc_buf != NULL) {
				struct aml_buf *aml_buf = (struct aml_buf *)afbc_buf->fb;
				pic->need_mmu_copy = 1;
				pic->mmu_copy_header_adr = aml_buf->fbc->haddr;

				hevc_print(hevc, PRINT_FLAG_VDEC_DETAIL,
					"CUR_POC(%d) mmu copy POC(%d), lcu_cnt %d decoder_tile_cnt %d, index %d\n",
					pic->POC, afbc_buf->POC, pic->lcu_cnt, pic->decoder_tile_cnt, index);
			}
		}

		clean_decoder_info(hevc, pic);
	}
	...
	if (hevc->lcu_percentage_threshold && (pic)) {
		pic->drop_flag = check_pic_decoded_lcu(hevc, pic);
...
static int check_ref_pic_drop_flag(struct hevc_state_s *hevc)
{
	int i = 0;
	struct PIC_s *pic = NULL;
	struct PIC_s *cur_pic = hevc->cur_pic;

	if (hevc->lcu_percentage_threshold == 0)
		return 0;
		...
		int decode_pic_begin)
{
	...
	hevc_print(hevc, H265_DEBUG_BUFMGR, "Col_ref %d, Col_POC %d\n", Col_ref, hevc->Col_POC);

	if (efficiency_mode == 0) {
		ref_pic_error_handle(hevc);
	} else {
#ifdef NEW_FB_CODE
		if (hevc->front_back_mode == 0 || hevc->front_back_mode == 2)
#endif
	...
	if (hevc->col_pic != NULL) {
		hevc_print(hevc, H265_DEBUG_BUFMGR,
			"hevc->col_pic->index %d, hevc->col_pic->poc %d, Col_POC %d\n",
			hevc->col_pic->index, hevc->col_pic->POC, hevc->Col_POC);
	}
	...
	if (check_ref_pic_drop_flag(hevc)) {
		hevc_print(hevc, H265_DEBUG_BUFMGR,
			"Drop_flag is 1, Discard this picture\n");
		return 5;
	}
...
static int H265_alloc_mmu(struct hevc_state_s *hevc, struct PIC_s *new_pic,
	...
	hevc_print(hevc, H265_DEBUG_BUFMGR,
		"%s pic index %d page count(%d) cur fb idx mmu %d dma 0x%lx ret =%d\n",
		__func__, new_pic->index,
...
static void set_canvas(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (pic->triple_write_mode) {
		...
		pic->tw_canvas_config[0].bit_depth	= is_tw_p010(hevc);
		...
		pic->tw_canvas_config[1].bit_depth	= is_tw_p010(hevc);

		ATRACE_COUNTER(hevc->trace.set_canvas0_addr, pic->tw_canvas_config[0].phy_addr);
		hevc_print(hevc, H265_DEBUG_PIC_STRUCT,"%s(tw_canvas0 addr:0x%x)\n",
			__func__, pic->tw_canvas_config[0].phy_addr);
...
static void v4l_vh265_fill_userdata(struct hevc_state_s *hevc,
	u8 *sei_data_buf, u32 data_len)
{
	...
	if (data_len > SEI_ITU_DATA_SIZE) {
		hevc_print(hevc, H265_DEBUG_PRINT_SEI,
			"%s, Size(%d) of this SEI packet not support\n", __func__);
		return;
	}
	memset(&usd_rep, 0, sizeof(struct sei_usd_param_s));
	tmp_buf = (char *)vzalloc(SEI_ITU_DATA_SIZE);
	if (tmp_buf == NULL) {
		hevc_print(hevc, H265_DEBUG_PRINT_SEI,
	...
	if (is_afd_data(tmp_buf)) {
		...
		usd_rep.meta_data.timestamp = hevc->cur_pic->timestamp;
		usd_rep.meta_data.vpts_valid = 1;
		usd_rep.meta_data.video_format = VFORMAT_HEVC;
		usd_rep.meta_data.poc_number = hevc->curr_POC;
		usd_rep.meta_data.frame_type = hevc->cur_pic->slice_type;
	...
	u32 vpts, int pts_valid, u32 poc)
{
	...
	v4l_vh265_fill_userdata(hevc, sei_data_buf, hevc->sei_itu_data_len);
		...
		if (p+payload_size <= sei_buf+size) {
			...
			case SEI_MasteringDisplayColorVolume:
				...
				for (i = 0; i < 3; i++) {
					for (j = 0; j < 2; j++) {
						...
						p_sei += 2;
					if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
						hevc_print_cont(hevc, 0, "primaries[%d][%d] %d \n", i, j, hevc->primaries[i][j]);
					}
				}
				for (i = 0; i < 2; i++) {
					hevc->white_point[i]
					...
					p_sei += 2;
				if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
					hevc_print_cont(hevc, 0, "white_point[%d] %d \n", i, hevc->white_point[i]);
				}
				for (i = 0; i < 2; i++) {
					hevc->luminance[i]
						= (*p_sei<<24)
						...
						| *(p_sei+3);
					if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
						hevc_print_cont(hevc, 0, "luminance[%d] %d ", i, hevc->luminance[i]);
					p_sei += 4;
				}
				if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
						hevc_print_cont(hevc, 0, "\n");
			...
			case SEI_ContentLightLevel:
				...
				hevc->content_light_level[1]
					= (*p_sei<<8) | *(p_sei+1);
				p_sei += 2;
				hevc->sei_hdr10_flag |= SEI_CONTENT_LIGHT_LEVEL_MASK;
				if (get_dbg_flag(hevc) & H265_DEBUG_PRINT_SEI)
					hevc_print_cont(hevc, 0,
...
static void v4l_vh265_report_hdr10p_data(struct hevc_state_s *hevc, struct PIC_s *pic,
	u8 *hdr10p_data_buf, u32 data_size)
{
	...
	if (pic->sei_present_flag & SEI_HDR10PLUS_MASK) {
		...
		usd_rep.meta_data.vpts_valid = 1;
		usd_rep.meta_data.video_format = VFORMAT_HEVC;
		usd_rep.meta_data.poc_number = pic->POC;
		usd_rep.meta_data.frame_type = pic->slice_type;
		usd_rep.meta_data.records_in_que =
			...
			struct PIC_s *pic)
{
	...
	if (pic->hdr10p_data_size > 0 && pic->hdr10p_data_buf)
		v4l_vh265_report_hdr10p_data(hevc, pic, pic->hdr10p_data_buf, pic->hdr10p_data_size);
...
static struct vframe_s *vh265_vf_get(void *op_arg)
{
	...
	if (kfifo_get(&hevc->display_q, &vf)) {
		...
		if (get_dbg_flag(hevc) & H265_DEBUG_PIC_STRUCT) {
			hevc_print(hevc, 0,
				"%s(vf 0x%p type %d index 0x%x poc %d/%d) pts(%d,%d) dur %d, discard_dv:%d\n",
...
static void h265_recycle_dec_resource(void *priv,
						struct aml_buf *aml_buf)
{
...
static int recycle_pending_vframe(struct hevc_state_s *hevc, struct vframe_s *vf)
{
	...
	hevc_print(hevc, H265_DEBUG_PIC_STRUCT,
		"%s warning(1), vf=>newframe_q: (index 0x%x)\n",
		__func__, vf->index);
...
static void v4l_submit_vframe(struct vdec_s *vdec);
...
/*SUPPORT_10BIT*/
		...
		if ((pic->pic_struct == 3 || pic->pic_struct == 4) && hevc->interlace_flag) {
			...
			if (pic->show_frame) {
				...
				kfifo_put(&hevc->newframe_q, (const struct vframe_s *)vf);
				kfifo_put(&hevc->newframe_q, (const struct vframe_s *)vf2);
				ATRACE_COUNTER(hevc->trace.new_q_name, kfifo_len(&hevc->newframe_q));
				return 0;
			}
			...
			if (pic->show_frame) {
				...
				kfifo_put(&hevc->newframe_q, (const struct vframe_s *)vf);
				kfifo_put(&hevc->newframe_q, (const struct vframe_s *)vf2);
				kfifo_put(&hevc->newframe_q, (const struct vframe_s *)vf3);
				ATRACE_COUNTER(hevc->trace.new_q_name, kfifo_len(&hevc->newframe_q));
				return 0;
			}
		...
		ATRACE_COUNTER(hevc->trace.disp_q_name, kfifo_len(&hevc->display_q));
		/*count info*/
		vdec_count_info(hevc->gvs, 2, stream_offset);
		...
		if (index <= MAX_REF_PIC_NUM)
			decoder_do_aux_data_check(vdec, hevc->m_PIC[index]->aux_data_buf,
				hevc->m_PIC[index]->aux_data_size, hevc->m_PIC[index]->POC);
#endif
...
static void v4l_submit_vframe(struct vdec_s *vdec)
{
	...
	mutex_lock(&hevc->fb_mutex);
#endif
	while (kfifo_peek(&hevc->display_q, &vf) && vf) {
		aml_buf = (struct aml_buf *)vf->v4l_mem_handle;
		index = vf->index & 0xff;
		if (index == 0xff)
			index = (vf->index >> 8) & 0xff;
		...
		if (pic->error_mark && hevc->lcu_percentage_threshold) {
			int ret = check_pic_decoded_lcu(hevc, pic);
			if (ret == 0) {
				pic->error_mark = 0;
				hevc_print(hevc, H265_DEBUG_BUFMGR,
		...
		if (pic->need_mmu_copy) {
			hevc_print(hevc, H265_DEBUG_BUFMGR,
				"Wait poc(%d)index %d mmu copy Complete\n", pic->POC, pic->index);
			break;
		}

#ifdef NEW_FB_CODE
		if (((hevc->front_back_mode) && (pic->back_done_mark)) ||
			(!hevc->front_back_mode)) {
#endif
			set_meta_data_to_vf(vf, UVM_META_DATA_VF_BASE_INFOS, hevc->v4l2_ctx);

			if ((pic->drop_flag) || (pic->error_mark && (hevc->PB_skip_mode != 0))) {
				vh265_vf_put(vh265_vf_get(vdec), vdec);
...
static void vh265_buf_ref_process_for_exception(struct hevc_state_s *hevc)
{
	struct aml_vcodec_ctx *ctx = (struct aml_vcodec_ctx *)(hevc->v4l2_ctx);

	if (hevc->decoding_pic && hevc->buf_allocated) {
...
irqreturn_t vh265_back_threaded_irq_cb(struct vdec_s *vdec, int irq)
{
	...
	if (dec_status == HEVC_BE_DECODE_DATA_DONE || hevc->front_back_mode == 2
		...
		if (((hevc->front_back_mode == 1 || hevc->front_back_mode == 3) &&
			get_double_write_mode(hevc) != 0x10) && (pic->need_mmu_copy != 1)) {
...
static irqreturn_t vh265_isr_thread_fn(int irq, void *data)
{
	...
	if (dec_status == HEVC_DECODE_BUFEMPTY ||
		...
		if (hevc->m_ins_flag) {
			...
			if (vdec_frame_based(hw_to_vdec(hevc))) {
				...
				if ((!hevc->first_pic_flag &&
					...
					(!hevc->slice_count || (hevc->cur_pic != NULL && hevc->buf_allocated)))
					&& (hevc->PB_skip_mode != 0)))
					vdec_v4l_post_error_frame_event(ctx);

				if (frmbase_multi_slice == 1)
					goto muti_output;
				else
					goto pic_done;
			} else {
		...
		if (hevc->m_ins_flag) {
			...
			if (hevc->cur_pic && (dec_status == HEVC_OVER_DECODE) && (hevc->buf_allocated == 1) &&
				hevc->chunk) {
				hevc->cur_pic->pts = hevc->chunk->pts;
				hevc->cur_pic->pts64 = hevc->chunk->pts64;
				hevc->cur_pic->timestamp = hevc->chunk->timestamp;
...
#ifdef NEW_FB_CODE
			if (hevc->front_back_mode && (hevc->frmbase_cont_flag == 0) && (hevc->slice_count)) {
				/*simulation code: if (dec_status == HEVC_DECPIC_DATA_DONE) {*/
...
muti_output:
			...
			if ((!input_stream_based(vdec) &&
					...
					if (hevc->mmu_enable
						...
						if (!hevc->m_ins_flag) {
							...
							if ((!is_skip_decoding(hevc, pic)) &&
								...
								== 1))
								recycle_mmu_buf_tail(hevc, false, hevc->cur_pic);
						}
					}
				...
				if (hevc->dec_result == DEC_RESULT_UNFINISH) {
					...
					vdec_schedule_work(&hevc->work);
					return IRQ_HANDLED;
				} else if ((frmbase_multi_slice != 1) || (hevc->slice_count != 0)) {
					check_pic_decoded_error(hevc,
						READ_VREG(HEVC_PARSER_LCU_START) & 0xffffff);
	...
	if (dec_status == HEVC_SEI_DAT) {
		...
		if (naltype == NAL_UNIT_EOS) {
			...
			/* eos is in the head of the chunk and followed by sps/pps/IDR
				...
				*/
			if (eos_in_head || input_stream_based(vdec)) {
...
/*#endif*/
		...
		if (hevc->pic_list_init_flag == 2) {
			...
			*/
				...
				if (ctx->param_sets_from_ucode && !hevc->v4l_params_parsed) {
					...
					if (hevc->front_back_mode == 1) {
						...
						int cur_mmu_4k_number = hevc->fb_ifbuf_num * hevc_mmu_page_num(hevc,
							hevc->pic_w, hevc->pic_h, bit_depth_chroma == 8);
					...
					vdec_parms_setup_and_sanity_check(hevc);
				}
			} else {
			...
			if (use_cma && (hevc->param.p.slice_segment_address == 0) &&
				...
				if (performance_profile && ((ret_is_csd_valid == RES_RET_NORMAL) && IS_8K_SIZE(hevc->pic_w,hevc->pic_h)))
					hevc->performance_profile = 1;
				else
					hevc->performance_profile = 0;
				hevc_print(hevc, 0, "hevc->performance_profile %d\n", hevc->performance_profile);
				if (hevc->pic_w == 0 || hevc->pic_h == 0 ||
					hevc->lcu_size == 0 || ret_is_csd_valid != RES_RET_NORMAL ||
					hevc_skip_nal(hevc)) {
					if (ret_is_csd_valid != RES_RET_NORMAL)
						hevc_print(hevc, 0, "%s, unsupported size : %u x %u. ret:%d\n",
		...
		h265_recycle_frame_buffer(hevc);
		if (ret < 0) {
#ifdef MULTI_INSTANCE_SUPPORT
			if (hevc->m_ins_flag) {
				vh265_buf_ref_process_for_exception(hevc);
			...
			if ((hevc->new_pic) && (hevc->cur_pic)) {
				...
				vdec_count_info(hevc->gvs, 2, hevc->curr_pic_offset);
			...
			if (!hevc->buf_allocated && hevc->cur_pic) {
				hevc->cur_pic->drop_mark = 1;
				vh265_report_err_timestamp_for_decoded_frames(ctx, hevc->cur_pic);
				hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,"%s prev frame poc %d ts %llu\n",
					__func__, hevc->cur_pic->POC, hevc->cur_pic->timestamp);
			}
	...
	if (efficiency_mode == 1) {
		if ((hevc->start_decoder_flag == 1) && (dec_status == HEVC_SLICE_SEGMENT_DONE)) {
			if (hevc->pic_list_init_flag == 3) {
#ifdef NEW_FB_CODE
				if (hevc->front_back_mode == 1 || hevc->front_back_mode == 3)
					init_pic_list_hw_fb(hevc);
...
static void vh265_check_timer_back_func(struct timer_list *timer)
{
	...
	if (decode_timeout_back_fast_check) {
		WRITE_VREG(HEVC_PATH_MONITOR_CTRL,
			(READ_VREG(HEVC_PATH_MONITOR_CTRL) & 0xfffffe0f) | 0x91);

		if (hevc->back_timer_check_count == 0) {
			hevc->back_timer_check_value = READ_VREG(HEVC_PATH_MONITOR_DATA);
			hevc->back_timer_check_count++;
		} else {
			int tmp_ddr_data = READ_VREG(HEVC_PATH_MONITOR_DATA);

			if (hevc->back_timer_check_value == tmp_ddr_data)
				hevc->back_timer_check_count++;
			else {
				hevc->back_timer_check_count = 1;
				hevc->back_timer_check_value = tmp_ddr_data;
			}
		}
	}
	...
	} else if ((decode_timeout_back_fast_check) &&
		(decode_timeout_val_back > 0) &&
		(hevc->start_process_time_back > 0) &&
		(hevc->back_timer_check_count >= decode_timeout_back_fast_check_threshold) &&
		(READ_VREG(HEVC_DEC_STATUS_DBE) != HEVC_BE_DECODE_DATA_DONE)) {
		hevc->back_timer_check_count = 0;
		timeout_process_back(hevc);
	}
...
#ifdef NEW_FRONT_BACK_CODE
	if (hevc->front_back_mode == 1)
		WRITE_VREG(HEVC_DECODE_COUNT, hevc->frontend_decoded_count);
...
static void timeout_process(struct hevc_state_s *hevc)
{
	...
	hevc_print(hevc, 0, "%s decoder timeout HEVC_MPC_E=0x%x\n",
...
static int h265_recycle_frame_buffer(struct hevc_state_s *hevc)
{
	...
	for (i = 0; i < hevc->used_buf_num && i < MAX_REF_PIC_NUM; ++i) {
		pic = hevc->m_PIC[i];
		if (pic == NULL)
			continue;
		if ((pic->referenced == 0) &&
			(pic->vf_ref || pic->drop_mark) &&
#ifdef NEW_FRONT_BACK_CODE
			(pic->backend_ref == 0) &&
#endif
			pic->cma_alloc_addr) {
			...
			hevc_print(hevc, H265_DEBUG_BUFMGR,
				"%s buf idx: %d pic index: %d poc %ddma addr: 0x%lx vb idx: %d vf_ref %d drop_mark %d\n",
				__func__, i, pic->index, pic->cma_alloc_addr, pic->POC,
			...
			} else if (pic->drop_mark && pic->vf_ref)
				hevc_print(hevc, H265_DEBUG_BUFMGR,
...
static void timeout_process_back(struct hevc_state_s *hevc)
{
	...
	if (work_pending(&hevc->work_back))
		return;
	hevc->dec_back_result = HEVC_BE_DECODE_TIMEOUT;
...
static unsigned char is_new_pic_available(struct hevc_state_s *hevc)
{
	struct aml_vcodec_ctx *ctx =
		(struct aml_vcodec_ctx *)(hevc->v4l2_ctx);
	struct PIC_s *pic;
	bool has_free_buf = is_available_buffer(hevc);
	int i;
	ulong flags;
...
static int vmh265_stop(struct hevc_state_s *hevc)
{
	...
	if ((hevc->ref_pic.dw_y_adr != 0) && (hevc->bmmu_box)) {
		decoder_bmmu_box_free_idx(hevc->bmmu_box, DEFAULT_REF_PIC_ID);
		hevc->ref_pic.dw_y_adr = 0;
	}
...
void clean_decoder_info(struct hevc_state_s *hevc, struct PIC_s* cur_pic)
{
	...
	if ((cur_pic->slice_type == 2) &&
		((cur_pic->POC < hevc->iPrevPOC) && (hevc->iPrevPOC != INVALID_POC))) {
		struct PIC_s *pic = NULL;
		struct afbc_buf *afbc_buf = NULL;

		for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
			afbc_buf = &hevc->afbc_buf_table[i];

			if (((afbc_buf->fb == 0) && (afbc_buf->used == 0))
				|| (afbc_buf->POC == INVALID_POC)
				|| (afbc_buf->POC == cur_pic->POC))
				continue;
		...
		hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
			"%s:cur pic is I, clean decoder poc info, pre(%d)\n",
			__func__, hevc->iPrevPOC);
	}
}

int find_near_pic_index(struct hevc_state_s *hevc, struct PIC_s* cur_pic)
{
	int i = 0;
	int index = -1;
	struct afbc_buf *afbc_buf = NULL;
	...
	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		...
		if ((afbc_buf->POC < cur_pic->POC)
			&& ((cur_pic->POC - afbc_buf->POC) < f_diff)) {
			f_diff = cur_pic->POC - afbc_buf->POC;
			f_index = i;
		}

		if ((cur_pic->POC < afbc_buf->POC)
			&& ((afbc_buf->POC - cur_pic->POC) < b_diff)) {
			b_diff = afbc_buf->POC - cur_pic->POC;
			b_index = i;
		}

		if ((cur_pic->POC < afbc_buf->POC)
			&& ((afbc_buf->POC - cur_pic->POC) > max_diff)) {
			max_diff = afbc_buf->POC - cur_pic->POC;
			max_index = i;
		}
	}
	...
	if (((hevc->dec_result == DEC_RESULT_GET_DATA) ||
		...
		if (run_ready(vdec, VDEC_HEVC)) {
			...
			if (get_dbg_flag(hevc) & PRINT_FRAMEBASE_DATA) {
				...
				if (!hevc->chunk->block->is_mapped)
					data = codec_mm_vmap(hevc->chunk->block->start + hevc->data_offset, r);
				else
					data = ((u8 *)hevc->chunk->block->start_virt) + hevc->data_offset;
		...
		mutex_lock(&hevc->fb_mutex);
#endif
		if ((hevc->cur_pic != NULL) && (hevc->cur_pic->error_mark)) {
		...
		if ((hevc->slice_count != 0) && (hevc->cur_pic != NULL)) {
			if (hevc->cur_pic->slice_type == I_SLICE) {
...
done_end:
		...
		*/
		...
		if (hevc->multi_frame_flag) {
			hevc->dec_result = DEC_RESULT_UNFINISH;
			hevc->data_size = hevc->data_size_bak;
			hevc->data_offset = hevc->data_offset_bak;
			hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
				"AGAIN, set unfinsh\n");
		}
...
void get_mmu_copy_map(struct hevc_state_s* hevc, struct PIC_s* pic, u32 page_num)
{
	int i = 0;
	u32 *map = (u32 *)hevc->frame_mmu_map_addr;
	u32 first_page = 0;
	u32 buffer_size = 0;
	int ret = 0;
	...
	if (buffer_size > mmu_copy->mmu_copy_buf_size) {
		start_time = div64_u64(local_clock(), 1000);
		if (mmu_copy->mmu_copy_buf_start && hevc->bmmu_box) {
			decoder_bmmu_box_free_idx(hevc->bmmu_box,
				MMU_COPY_IDX(aml_buf->fbc->index));
			mmu_copy->mmu_copy_buf_size = 0;
		}
		...
		ret = decoder_bmmu_box_alloc_buf_phy(hevc->bmmu_box,
			MMU_COPY_IDX(aml_buf->fbc->index),
			buffer_size, DRIVER_NAME, &mmu_copy->mmu_copy_buf_start);
		if (ret < 0) {
			mmu_copy->mmu_copy_buf_start = 0;
			pic->need_mmu_copy = 0;
			hevc_print(hevc, 0,
		...
		mmu_copy->mmu_copy_buf_size = buffer_size;
	...
	if (mmu_copy->mmu_copy_buf_start) {
		first_page = mmu_copy->mmu_copy_buf_start >> PAGE_SHIFT;
	} else {
		pic->need_mmu_copy = 0;
		hevc_print(hevc, 0,
	...
	for (i = 0; i < page_num - 1; i++) {
		map[i] = (mmu_copy->mmu_copy_buf_start + i * PAGE_SIZE) >> PAGE_SHIFT;
...
static void error_handle_mmu_copy(struct hevc_state_s *hevc, PIC_t* pic)
{
	...
	struct mmu_copy_params params2;
	int x_location = 0;
	int y_location = 0;
	unsigned int mmu_copy_4k_num = 0;
	u16 bit_depth = hevc->param.p.bit_depth;
	u32 page_num = 0;
	int tile_x = pic->decoder_tile_cnt % hevc->num_tile_col;
	int tile_y = pic->decoder_tile_cnt / hevc->num_tile_col;

	if ((pic->tile_cnt == 1) || (pic->decoder_tile_cnt < 1)) {
		int lcu_y = pic->lcu_cnt / hevc->lcu_x_num;

		if ((pic->tile_cnt == 1) && (lcu_y > 1)) {
			y_location = (lcu_y * hevc->lcu_size - 8) >> 3 << 3;
		}

		hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
			"lcu_cnt %d, lcu_x_num %d, lcu_size %d\n",
			pic->lcu_cnt, hevc->lcu_x_num, hevc->lcu_size);

		params.pic_w = hevc->pic_w;
		params.pic_h = hevc->pic_h;
		params.x_location = x_location;
		params.y_location = y_location;
		params.err_width = ALIGN((hevc->pic_w - x_location), 64);
		params.err_height = ALIGN((hevc->pic_h - y_location), 8);
		...
		page_num = hevc_mmu_page_num(hevc, params.err_width, params.err_height, bit_depth == 0);
	} else {
		x_location = hevc->m_tile[tile_y][tile_x].start_cu_x * hevc->lcu_size;
		y_location = hevc->m_tile[tile_y][tile_x].start_cu_y * hevc->lcu_size;

		if (x_location > 8)
			x_location = (x_location - 8) >> 6 << 6;
		else
			x_location = 0;

		if (y_location > 8)
			y_location = (y_location - 8) >> 3 << 3;
		else
			y_location = 0;

		hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
			"tile_x %d tile_y %d, decoder_tile_cnt %d, num_tile_col %d lcu_size %d\n",
			tile_x, tile_y, pic->decoder_tile_cnt,
			hevc->num_tile_col, hevc->lcu_size);
		...
		hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
			"Pic Size(%d, %d), Copy start pos(%d, %d), Copy Size1(%d, %d)\n",
			params.pic_w, params.pic_h, params.x_location,
			params.y_location, params.err_width, params.err_height);

		page_num = hevc_mmu_page_num(hevc, params.err_width, params.err_height, bit_depth == 0);

		if ((tile_x != 0) && (tile_y + 1 != hevc->num_tile_row)) {
			tile_y += 1;

			y_location = hevc->m_tile[tile_y][0].start_cu_y * hevc->lcu_size;
			...
			params2.pic_w = hevc->pic_w;
			params2.pic_h = hevc->pic_h;
			params2.x_location = 0;
			params2.y_location = y_location;
			params2.err_width = x_location;
			params2.err_height = ALIGN((hevc->pic_h - y_location), 8);

			hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
				"Pic Size(%d, %d), Copy start pos(%d, %d), Copy Size2(%d, %d)\n",
				params2.pic_w, params2.pic_h, params2.x_location,
				params2.y_location, params2.err_width, params2.err_height);

			page_num += hevc_mmu_page_num(hevc, params2.err_width, params2.err_height, bit_depth == 0);
		}
	}

	get_mmu_copy_map(hevc, pic, page_num);
	...
	params2.mmu_copy_map_phy_addr = hevc->frame_mmu_map_phy_addr;
	params2.mmu_copy_err_header_adr = pic->header_adr;
	params2.mmu_copy_pre_header_adr = pic->mmu_copy_header_adr;
	...
	if ((pic->tile_cnt == 1) || (pic->decoder_tile_cnt < 1)) {
		...
		if (mmu_copy_4k_num > page_num) {
			pic->need_mmu_copy = 0;
			return ;
		}

		memmove(hevc->frame_mmu_map_addr,
			hevc->frame_mmu_map_addr + mmu_copy_4k_num * 4,
			(page_num - mmu_copy_4k_num) * 4);

		if ((tile_x != 0) && (tile_y + 1 != hevc->num_tile_row)) {
			mmu_copy_work(params2);
		}
	}
	pic->need_mmu_copy = 0;
}
	...
	struct vdec_s *vdec,int from)
{
	...
	if (hevc->dec_back_result == HEVC_BE_DECODE_TIMEOUT) {
		mutex_lock(&hevc->fb_mutex);
		if (pic->error_mark == 0) {
			hevc->gvs->error_frame_count++;
			if (pic->slice_type == I_SLICE) {
	...
	pic_backend_ref_operation(hevc, 0);
...
static unsigned long run_ready(struct vdec_s *vdec, unsigned long mask)
{
	...
	if (!hevc->first_sc_checked && hevc->mmu_enable) {
		...
		if (ctx->front_back_mode)
			mmu_box1 = ctx->bm.mmu_1;
#endif

		size = decoder_mmu_box_sc_check(mmu_box, tvp);
#ifdef NEW_FB_CODE
		if (ctx->front_back_mode)
			size_1 = decoder_mmu_box_sc_check(mmu_box1, tvp);
#endif
	...
	if (ctx->param_sets_from_ucode) {
		if (hevc->v4l_params_parsed) {
			ret = is_available_buffer(hevc) ? 1 : 0;
		} else {
	...
	void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	if (vdec_frame_based(vdec)) {
		WRITE_VREG(HEVC_SHIFT_BYTE_COUNT, 0);
		r = hevc->data_size + (hevc->data_offset & (VDEC_FIFO_ALIGN - 1));
		hevc->decode_size = r;
		if (vdec->mvfrm)
...
static void reset(struct vdec_s *vdec)
{
	...
	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		if (hevc->afbc_buf_table[i].used) {
			...
			hevc->afbc_buf_table[i].POC = INVALID_POC;
		}
	}
...
static int ammvdec_h265_probe(struct platform_device *pdev)
{
	...
	if (((get_dbg_flag(hevc) & IGNORE_PARAM_FROM_CONFIG) == 0) &&
		...
		if (get_config_int(pdata->config, "hevc_triple_write_mode",
			&config_val) == 0)
			hevc->triple_write_mode = config_val;
		else
			hevc->triple_write_mode = triple_write_mode;
		...
		if (get_config_int(pdata->config,
			...
			hevc->dv_duallayer = config_val & VDEC_CFG_FLAG_DV_TWOLAYER;
			if (config_val & VDEC_CFG_FLAG_MMU_COPY_DISABLE) {
				hevc->mmu_copy_disable = true;
		...
		if (get_config_int(pdata->config,
			...
			hevc->lcu_percentage_threshold = config_val & 0xff;
		}
	...
	hevc->lcu_percentage_threshold = hevc->lcu_percentage_threshold % 101;
	if (hevc->lcu_percentage_threshold > 20) {
	...
	if (is_dw_p010(hevc) || is_tw_p010(hevc))
...
static int __init amvdec_h265_driver_init_module(void)
{
	...
	register_set_debug_flag_func(DEBUG_AMVDEC_H265_FB_V4L, set_debug_flag);
	...
	vcodec_profile_register_v2("H.265-V4L-FB", VFORMAT_HEVC, 1);
	INIT_REG_NODE_CONFIGS("media.decoder", &decoder_265_node,
		"h265-v4l-fb", h265_configs, CONFIG_FOR_RW);

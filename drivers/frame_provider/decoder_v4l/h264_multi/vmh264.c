...
static unsigned int mb_count_threshold = 0; /*percentage*/
...
static int is_crop_valid(struct vdec_h264_hw_s *hw, int mb_width, int mb_height, u32 param2, u32 *p_crop_right, u32 *p_crop_bottom)
{
	...
	*p_crop_right = crop_right;
	*p_crop_bottom = crop_bottom;
...
static enum ResResult is_csd_valid(struct vdec_h264_hw_s *hw, int mb_width, int mb_height, u32 param2, u32 param4) {
	...
	crop_valid = is_crop_valid(hw, mb_width, mb_height, param2, &curr_info.crop_right, &curr_info.crop_bottom);
	...
	if (over_size == RES_RET_OVERSIZE) {
		if (hw->old_csd_info_check_count == 0) {
			hw->old_csd_info_check_count++;
			memcpy(&hw->old_csd_info, &curr_info, sizeof(struct mh264_csd_main_info_t));
		} else {
			if (!memcmp(&hw->old_csd_info, &curr_info, sizeof(struct mh264_csd_main_info_t)))
				hw->old_csd_info_check_count++;
			else {
				hw->old_csd_info_check_count = 1; // reset to 0+1
				memcpy(&hw->old_csd_info, &curr_info, sizeof(struct mh264_csd_main_info_t));
			}
		}

		if (hw->old_csd_info_check_count >= 3)
...
static struct aml_buf *index_to_afbc_aml_buf(struct vdec_h264_hw_s *hw, int index)
{
	...
	for (i = 0; i < BUF_FBC_NUM_MAX; i++) {
		if (hw->buffer_spec[index].cma_alloc_addr
...
static void hevc_mcr_config_canv2axitbl(struct vdec_h264_hw_s *hw, int restore)
{
	...
	for (i = 0; i < num_buff; i++) {
		if (is_vdec_hevc_combine())
			WRITE_VREG((canvas_addr + i), (i << 1) | (i << 9) | (i << 17));
		else
	...
	for (i = 0; i < num_buff; i++) {
		...
		if (dw_mode) {
			hw->buffer_spec[i].dw_y_adr = hw->buffer_spec[i].y_addr;
			hw->buffer_spec[i].dw_u_v_adr = hw->buffer_spec[i].u_addr;
		}
	}
...
static void hevc_mcr_config_mc_ref(struct vdec_h264_hw_s *hw)
{
	...
	for (i = 0; i < (unsigned int)(pSlice->listXsize[0]); i++) {
		...
		if (is_vdec_hevc_combine())
			ref_canv = (READ_VREG(CURR_CANVAS_CTRL)&0xffffff) >> 1;
		else
...
static void hevc_sao_set_pic_buffer(struct vdec_h264_hw_s *hw,
	...
	ret = hevc_alloc_mmu(hw, pic->buf_spec_num, hw->frame_mmu_map_addr);
...
static void  hevc_set_unused_4k_buff_idx(struct vdec_h264_hw_s *hw,
	...
	if (is_vdec_hevc_combine())
		hw->hevc_cur_buf_idx = (READ_VREG(CURR_CANVAS_CTRL)&0xff) >> 1;
	else
...
{
	memcpy(vf, vh264_vf_get(vdec_ctx), sizeof(struct vframe_s));
}
...
static int v4l_alloc_buf(struct vdec_h264_hw_s *hw, int idx)
{
	...
	bs->cma_alloc_addr = (unsigned long)hw->aml_buf;
	bs->buf_adr = hw->aml_buf->planes[0].addr;
	if (hw->mmu_enable) {
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_V4L_DETAIL,
		"[%d] %s(), aml_buf:%lx, mmu enable:%d, haddr:%lx\n",
		ctx->id, __func__,
		hw->mmu_enable,
		bs->cma_alloc_addr,
		bs->alloc_header_addr);

	if (ctx->avbcd_work_mode)
		return 0;

	if (hw->aml_buf->num_planes == 1) {
		y_addr = hw->aml_buf->planes[0].addr;
		c_addr = hw->aml_buf->planes[0].addr + hw->aml_buf->planes[0].offset;
		hw->aml_buf->planes[0].bytes_used = hw->aml_buf->planes[0].length;
	} else if (hw->aml_buf->num_planes == 2) {
		y_addr = hw->aml_buf->planes[0].addr;
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_V4L_DETAIL,
		"[%d] %s(), y_addr: %x, size: %u\n",
		ctx->id, __func__, y_addr, hw->aml_buf->planes[0].length);
	dpb_print(DECODE_ID(hw), PRINT_FLAG_V4L_DETAIL,
		"[%d] %s(), c_addr: %x, size: %u\n",
		ctx->id, __func__, c_addr, hw->aml_buf->planes[1].length);
...
static void config_decode_canvas(struct vdec_h264_hw_s *hw, int i)
{
	...
	if (is_vdec_hevc_combine()) {
		WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR,
			(hw->buffer_spec[i].y_canvas_index << 8) | (1 << 1));
		WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_DATA,
			hw->buffer_spec[i].y_addr >> 5);
	} else {
		if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_G12A) {
	...
	if (is_vdec_hevc_combine()) {
		WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR,
			(hw->buffer_spec[i].u_canvas_index << 8) | (1 << 1));
		WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_DATA,
			hw->buffer_spec[i].u_addr >> 5);

		WRITE_VREG(HEVCD_MPP_ANC_CANVAS_ACCCONFIG_ADDR,
			(hw->buffer_spec[i].y_canvas_index << 7) | 1);
		WRITE_VREG(HEVCD_MPP_ANC_CANVAS_DATA_ADDR,
			(hw->buffer_spec[i].u_canvas_index << 8) | hw->buffer_spec[i].y_canvas_index);
	} else {
		if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_G12A) {
			WRITE_VREG(VDEC_ASSIST_CANVAS_BLK32,
					(1 << 11) |
...
static void config_canvas_hevc(struct vdec_h264_hw_s *hw)
{
...
int v4l_get_free_buf_idx(struct vdec_s *vdec)
{
	...
	for (i = 0; i < hw->dpb.mDPB.size; i++) {
		...
		if ((!v4l->avbcd_work_mode && ((pic->used == 0 || pic->used == -1) &&
			pic->vf_ref == 0 &&
			!pic->cma_alloc_addr)) ||
			(v4l->avbcd_work_mode && hw->aml_buf &&
			(pic->am_buf == hw->aml_buf))) {
			if (v4l_alloc_buf(hw, i)) {
				spin_unlock_irqrestore(&hw->bufspec_lock, flags);
				return INVALID_IDX;
			}
	...
	if (idx != INVALID_IDX) {
		if (hw->mmu_enable && hw->config_bufmgr_done) {
	...
	if (idx < 0) {
		...
		for (i = 0; i < hw->dpb.mDPB.size; i++) {
			...
			"%s spec idx: %d used: %d vf_ref: %d cma_alloc_addr: 0x%lx\n",
			__func__, i , pic->used, pic->vf_ref, pic->cma_alloc_addr);
		}
						...
						int frame_index)
{
	...
	if (!aml_buf) {
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
		"fb(buf_spec_num: %d) has been recycled!\n",
		__func__, buf_spec_num);
		return 0;
	}
	dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
		"%s buf_spec_num %d frame_index %d used %d cma_alloc_addr 0x%lx\n",
		__func__, buf_spec_num, frame_index,
		buf_spec_num > 0 ? hw->buffer_spec[buf_spec_num].used : 0,
	...
	set_frame_output_flag(&hw->dpb, p_H264_Dpb->mDPB.fs[frame_index]->index);
...
static void h264_avbc_done_cb(void *output)
{
	...
	if (hw->buffer_spec[buf_spec_num].used == 2) {
		struct h264_dpb_stru *p_H264_Dpb = &hw->dpb;
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
		"%s %p(%d) to fs[%d], poc %d buf_spec_num %d used %d vf_ref %d ts:%llu\n",
		__func__, vf, vf->index, frame_index,
...
static void h264_post_avbcd_task(struct vdec_h264_hw_s *hw)
{
	...
	int offset;
	int bitdepth = ctx->picinfo.bitdepth;
	int align_w = 64;
	int align_h = 64;
	...
	if (!am_buf) {
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS, "avbc no frame buffers!\n");
		goto out;
	}
	...
	if (is_hevc_align32(0))
		align_w = 32;
	vf->canvas0_config[0].block_mode = hw->canvas_mode;
	vf->canvas0_config[0].endian = 0;
	vf->canvas0_config[1].block_mode = hw->canvas_mode;
	...
	out->done_func = h264_avbc_done_cb;

	aml_buf_done(&ctx->bm, dec_buf, BUF_USER_DEC);

	dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
			"%s: block mode 0x%x bit_depth_luma %d (vf %px header_addr 0x%x y_addr 0x%lx wxh %d x %d bitdepth %d type 0x%lx index %d"
			" offset %d)\n",
			__func__, hw->canvas_mode, bitdepth, vf, in->img.data, am_buf->planes[0].addr, in->img.rect.width, in->img.rect.height, in->img.bitdep,
	...
	unsigned char release_all)
{
	...
	if (dealloc_flag && dpb_is_debug(DECODE_ID(hw), PRINT_FLAG_DUMP_BUFSPEC))
		dump_bufspec(hw, __func__);
	return;
}

unsigned char is_there_free_buffer(struct vdec_s *vdec)
{
	int i;
	struct vdec_h264_hw_s *hw = (struct vdec_h264_hw_s *)vdec->private;
	struct aml_vcodec_ctx * ctx = hw->v4l2_ctx;
	...
	for (i = 0; i < hw->dpb.mDPB.size; i++) {
		if ((hw->buffer_spec[i].used == 0 || hw->buffer_spec[i].used == -1) &&
			hw->buffer_spec[i].vf_ref == 0 &&
			((!ctx->avbcd_work_mode && !hw->buffer_spec[i].cma_alloc_addr) ||
			ctx->avbcd_work_mode)){
			free_slot++;
			if (ctx->avbcd_work_mode)
	...
	if (ctx->bm.config.avbcd_work_mode) {
		...
		if (!hw->aml_buf) {
			if (!pic->buffer_attached) {
				...
				pic->am_buf->task->attach(pic->am_buf->task, &task_dec_ops,  hw_to_vdec(hw));
				pic->am_buf->state = FB_ST_DECODER;
				pic->buffer_attached = true;
			}
			hw->aml_buf = pic->am_buf;
			dpb->avbcd_work_mode = ctx->avbcd_work_mode;
		} else
			dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
...
out:
	...
	if (free_count < run_ready_min_buf_num_active) {
		...
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
			...
			__func__, hw->aml_buf, free_count);
	}
...
static int post_prepare_process(struct vdec_s *vdec, struct FrameStore *frame)
{
	...
	if (buffer_index < 0 || buffer_index >= BUFSPEC_POOL_SIZE) {
		dpb_print(DECODE_ID(hw), 0, "%s, buffer_index 0x%x is beyond range\n",
...
static void put_vf_to_avbc_q(struct vdec_h264_hw_s *hw, struct vframe_s *vf)
{
...
static int post_video_frame(struct vdec_s *vdec, struct FrameStore *frame)
{
	...
	for (i = 0; i < vf_count; i++) {
		...
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
			"%s: vf %px, index %d poc %d frame_type %d dur %d type %x pts %d(0x%x), pts64 %lld(0x%x) ts %lld(0x%x)\n",
			__func__, vf, vf->index, frame->poc, vf->frame_type, vf->duration, vf->type, vf->pts, vf->pts,
...
int notify_v4l_eos(struct vdec_s *vdec)
{
	...
	if (hw->eos) {
		...
		if (ctx->avbcd_work_mode)
			aml_buf = hw->buffer_spec[index].am_buf;
		else
	...
	unsigned char dv_meta_flag, struct vdec_h264_hw_s *hw_b)
{
	...
	unsigned size_reg_val = READ_VREG(H264_AUX_DATA_SIZE);
	...
	if (suffix_flag) {
		aux_adr = (unsigned short *)(hw_buf->aux_addr + hw_buf->prefix_aux_size);
		aux_count = ((size_reg_val & 0xffff) << 4) >> 1;
		aux_size = hw_buf->suffix_aux_size;
	} else {
		aux_adr = (unsigned short *)hw_buf->aux_addr;
		aux_count = ((size_reg_val >> 16) << 4) >> 1;
		aux_size = hw_buf->prefix_aux_size;
	}
	if (dpb_is_debug(DECODE_ID(hw), PRINT_FLAG_SEI_DETAIL)) {
		dpb_print(DECODE_ID(hw), 0,
...
static void dump_aux_buf(struct vdec_h264_hw_s *hw)
{
	...
	unsigned aux_size = (READ_VREG(H264_AUX_DATA_SIZE) >> 16) << 4;

	if (hw->prefix_aux_size > 0) {
		dpb_print(DECODE_ID(hw), 0, "prefix aux: (size %d)\n", aux_size);
	...
	if (hw->suffix_aux_size > 0) {
		...
		aux_size = (READ_VREG(H264_AUX_DATA_SIZE) & 0xffff) << 4;
		dpb_print(DECODE_ID(hw),
			0, "suffix aux: (size %d)\n", aux_size);
		for (i = 0; i < (aux_size >> 1); i++) {
			dpb_print_cont(DECODE_ID(hw),
...
#define H264_CO_MB_RW_CTL         VLD_C3D /* 0xc3d */
	...
	colocate_adr_offset = ((pic->structure == FRAME && pic->mb_aff_frame_flag == 0)
		 ? 1 : 2) * 96;
	if (use_direct_8x8)
...
static struct vframe_s *vh264_vf_get(void *op_arg)
{
	...
	if ((!v4l2_ctx->avbcd_work_mode && kfifo_get(&hw->display_q, &vf)) ||
		(v4l2_ctx->avbcd_work_mode && kfifo_get(&hw->avbc_display_q, &vf))) {
		int time = jiffies;
		...
		if (dpb_is_debug(DECODE_ID(hw),
			...
			if (frame_index < 0 ||
					frame_index >= DPB_SIZE_MAX) {
				dpb_print(DECODE_ID(hw), 0,
						"%s vf %px index 0x%x error\r\n",
						__func__, vf, vf->index);
			} else {
				dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_DETAIL,
				"%s buf_spec_num %d vf %px poc %d dur %d pts %d interval %dms, ts: %lld\n",
...
static void h264_recycle_dec_resource(void *priv,
						struct aml_buf *aml_buf)
{
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
		"%s buf_spec_num: %d dma addr:0x%lx fb idx: %d\n",
		__func__, buf_spec_num,
		hw->buffer_spec[buf_spec_num].buf_adr, aml_buf->index);
	...
	u32 param1, u32 param2, u32 param3, u32 param4, bool buffer_reset_flag, bool reset_flags)
{
	...
	if (ret_is_csd_valid) {
		dpb_print(DECODE_ID(hw), 0,
			"!!!wrong seq_info2 0x%x mb_width/mb_height (0x%x/0x%x), ret:%d\r\n",
			seq_info2,
			mb_width,
			mb_height,
			ret_is_csd_valid);
	...
	if (hw->config_bufmgr_done == 0) {
		...
		hw->mb_width_hevc = mb_width;
		hw->mb_height_hevc = mb_height;
		if (!hw->mmu_enable) {
		...
		if (!hw->res_ch_flag) {
			hw->dpb_frames_bak = pic.dpb_frames;
			hw->dpb_margin_bak = pic.dpb_margin;
		}
		hw->dpb.mDPB.size = active_buffer_spec_num;
		if (ctx->avbcd_work_mode)
			hw->dpb.mDPB.size = pic.dpb_frames;
	...
	if (hw->timing_info_present_flag) {
		...
		if (((hw->num_units_in_tick * 120) >= hw->time_scale &&
			...
			if (hw->use_idr_framerate ||
				...
				u32 frame_dur_es =
					div_u64(96000ULL * 2 * hw->num_units_in_tick, hw->time_scale);
						...
						if (hw->loop_flag >= loop_times) {
							for (i = 0; i < p_Dpb->used_size; i++) {
								if ((hw->loop_last_poc + loop_playback_poc_threshold < p_Dpb->fs[i]->poc) &&
									...
									p_Dpb->fs[i]->is_output, p_Dpb->fs[i]->pre_output,
...
#define VLD_RST_RANGE     (0xf << 0)
#define RESET_VLD_INI     BIT(0)
#define RESET_VLD_ERR     BIT(1)
#define RESET_VLD_DEC     BIT(2)

#define MC_RST_RANGE      (0xf << 4)
#define RESET_MC_INI      BIT(4)
#define RESET_MC_ERR      BIT(5)
#define RESET_MC_DEC      BIT(6)

#define DBLK_RST_RANGE    (0xf << 8)
#define RESET_DBLK_INI    BIT(8)
#define RESET_DBLK_ERR    BIT(9)
#define RESET_DBLK_DEC    BIT(10)

#define IQIDCT_RST_RANGE  (0xf << 12)
#define RESET_IQIDCT_INI  BIT(12)
#define RESET_IQIDCT_ERR  BIT(13)
#define RESET_IQIDCT_DEC  BIT(14)

#define PIC_DC_RST_RANGE  (0xf << 16)
#define RESET_PIC_DC_INI  BIT(16)
#define RESET_PIC_DC_ERR  BIT(17)
#define RESET_PIC_DC_DEC  BIT(18)

#define EXTIF_RST_RANGE (0xf << 20)

#define MMU_RST_RANGE   (0xf << 24)

#define nop_delay(n) do {nop_cnt = (n); while(nop_cnt--); } while (0)

static void h264_idle_axi_reset(struct vdec_h264_hw_s *hw, char *str, int reset_bit)
{
	int vcop_ctrl, iq_quant, iqidct_ctrl, rv_ai_mb_count;
	int ipic_mb_count, dcac_dma_ctrl, dcac_dma_buf_start, dcac_ddr_byte;
	int co_mb_adr, pwr_ctrl, mb_width;
	int nop_cnt;
	...
	if (!reset_bit) {
		dpb_print(DECODE_ID(hw), 0, "%s in, no reset bit return\n", __func__);
		return;
	}

	dpb_print(DECODE_ID(hw), PRINT_FLAG_UCODE_EVT,
		"%s, %s, mby_mbx %x, rstbit 0x%x\n",
		__func__, str, READ_VREG(MBY_MBX), reset_bit);

	arb_ctrl_wait_idle(0);

	if (reset_bit & VLD_RST_RANGE) {
		co_mb_adr = READ_VREG(H264_CO_MB_WR_ADDR);
		pwr_ctrl = READ_VREG(POWER_CTL_VLD);
		mb_width = READ_VREG(MB_WIDTH);

		if (reset_bit & RESET_VLD_ERR) {
		...
		WRITE_VREG(H264_CO_MB_WR_ADDR, co_mb_adr);
		WRITE_VREG(POWER_CTL_VLD, pwr_ctrl);
		WRITE_VREG(MB_WIDTH, mb_width);
	}

	if (reset_bit & IQIDCT_RST_RANGE) {
		vcop_ctrl = READ_VREG(VCOP_CTRL_REG);
		iq_quant  = READ_VREG(NON_I_QUANT_MATRIX);  //IQ_QUANT_USE_DEFAULT
		iqidct_ctrl = READ_VREG(IQIDCT_CONTROL);
		rv_ai_mb_count = READ_VREG(RV_AI_MB_COUNT);

		ipic_mb_count = READ_VREG(I_PIC_MB_COUNT_HW);
		dcac_dma_ctrl = READ_VREG(DCAC_DMA_HW_CTL);
		dcac_dma_buf_start = READ_VREG(DCAC_DMA_HW_BUFF_START);
		dcac_ddr_byte = READ_VREG(DCAC_DDR_BYTE64_CTL);

		/*reset 6: iqidct */
		WRITE_VREG(DOS_SW_RESET0, (1 << 6));
		WRITE_VREG(DOS_SW_RESET0, 0);
		...
		WRITE_VREG(NON_I_QUANT_MATRIX, iq_quant);
		WRITE_VREG(IQIDCT_CONTROL, iqidct_ctrl);
		WRITE_VREG(RV_AI_MB_COUNT, rv_ai_mb_count);
		WRITE_VREG(I_PIC_MB_COUNT_HW, ipic_mb_count);
		WRITE_VREG(DCAC_DMA_HW_CTL, dcac_dma_ctrl | (0x3 << 29));
		WRITE_VREG(DCAC_DMA_HW_BUFF_START, dcac_dma_buf_start);
		WRITE_VREG(DCAC_DDR_BYTE64_CTL, dcac_ddr_byte);
	}

	if (reset_bit & MC_RST_RANGE) {
		if (reset_bit & RESET_MC_INI) {
			SET_VREG_MASK(MC_CTRL1, ((1 << 0) | (1 << 3)));   //flush
			nop_delay(10);
			CLEAR_VREG_MASK(MC_CTRL1, ((1 << 0) | (1 << 3)));
		} else if (reset_bit & RESET_MC_ERR) {
			SET_VREG_MASK(MC_CTRL1, ((1 << 0) | (1 << 2)));  //reset
			nop_delay(10);
			CLEAR_VREG_MASK(MC_CTRL1, ((1 << 0) | (1 << 2)));
		}
		nop_delay(10);
	}

	if (reset_bit & DBLK_RST_RANGE) {
		if (reset_bit & RESET_DBLK_INI) {
			WRITE_VREG(DBLK_RST, 0x7);
			nop_delay(10);
			WRITE_VREG(DBLK_RST, 0);
		} else if (reset_bit & RESET_DBLK_ERR) {
			WRITE_VREG(DBLK_RST, 0x17);
			nop_delay(10);
			WRITE_VREG(DBLK_RST, 0);
		}
		nop_delay(10);
	...
	if (reset_bit & PIC_DC_RST_RANGE) {
		if (reset_bit & RESET_PIC_DC_INI) {
			SET_VREG_MASK(MDEC_PIC_DC_CTRL, (0x3f << 24));
			nop_delay(10);
			CLEAR_VREG_MASK(MDEC_PIC_DC_CTRL, (0x3f << 24));

		} else if (reset_bit & RESET_PIC_DC_ERR) {

			dos_wait_status(MDEC_PIC_DC_STATUS, 0xffffff, 0);
			dos_wait_status(H264_CO_MB_RW_CTL, (3 << 10), 0);

			SET_VREG_MASK(MDEC_PIC_DC_CTRL, ((0x3f << 24) | (1 << 0)));
			nop_delay(10);
			CLEAR_VREG_MASK(MDEC_PIC_DC_CTRL, ((0x3f << 24) | (1 << 0)));
			nop_delay(10);
			WRITE_VREG_BITS(MDEC_PIC_DC_THRESH, 0x7f, 8, 8);
		}
	}

	if (reset_bit & EXTIF_RST_RANGE) {
		SET_VREG_MASK(MDEC_EXTIF_CFG1, 1 << 30);
		CLEAR_VREG_MASK(MDEC_EXTIF_CFG1, 1 << 30);
	}

	if (reset_bit & MMU_RST_RANGE) {
		/*
		 * 2: assist
		 * 3: parser
		 * 4: parser_state
...
static int h264_reset_reinit_mmu(struct vdec_h264_hw_s *hw)
{
	if (!is_vdec_hevc_combine())
		return 0;

	if (!hw->mmu_enable) {
		h264_idle_axi_reset(hw, "reset_reinit non-mmu",
			RESET_IQIDCT_ERR | RESET_MC_ERR | RESET_DBLK_ERR | RESET_PIC_DC_ERR);
		return 0;
	}

	h264_idle_axi_reset(hw, "reset_reinit mmu",
		RESET_IQIDCT_ERR | RESET_MC_ERR | RESET_DBLK_ERR | RESET_PIC_DC_ERR |
		EXTIF_RST_RANGE | MMU_RST_RANGE);
	...
	if (hw->config_bufmgr_done) {
		...
		for (i = 0; i < num_buff; i++) {
			if (is_vdec_hevc_combine())
				WRITE_VREG((ANC0_CANVAS_ADDR + i), (i << 1) | (i << 9) | (i << 17));
			else
				WRITE_VREG((ANC0_CANVAS_ADDR + i), i | (i << 8) | (i << 16));
		}
		WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR, (0x1 << 1) | (0x1 << 2));
		...
		for (j = 0; j < num_buff; j++) {
			...
			WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_DATA,	maddr >> 5);
			dpb_print(DECODE_ID(hw), PRINT_FLAG_MMU_DETAIL,
				"%s : canvas: %d  axiaddr:%x\n",
				__func__, i, (u32)maddr);
		}
...
static irqreturn_t vh264_isr_thread_fn(struct vdec_s *vdec, int irq)
{
	...
	if (is_vdec_hevc_combine()) {
		int reset_bit = 0;
		if ((dec_dpb_status & 0xff) == H264_DECODE_ERROR_RESET) {
			if (dec_dpb_status & 0x100)
				reset_bit |= RESET_VLD_ERR;
			if (dec_dpb_status & 0x200)
				reset_bit |= RESET_MC_ERR;
			if (dec_dpb_status & 0x400)
				reset_bit |= RESET_DBLK_ERR;
			if (dec_dpb_status & 0x800)
				reset_bit |= RESET_IQIDCT_ERR;
			...
			if (dec_dpb_status & 0x100)
				reset_bit |= RESET_VLD_INI;
			if (dec_dpb_status & 0x200)
				reset_bit |= RESET_MC_INI;
			if (dec_dpb_status & 0x400)
				reset_bit |= RESET_DBLK_INI;
			if (dec_dpb_status & 0x800)
				reset_bit |= RESET_IQIDCT_INI;
			if (dec_dpb_status & 0x1000)
				reset_bit |= RESET_PIC_DC_INI;

			h264_idle_axi_reset(hw, "init reset", reset_bit);

			WRITE_VREG(DPB_STATUS_REG, 0);
			return IRQ_HANDLED;
		}
	} else {
		if (dec_dpb_status & 0xff00) {
			dpb_print(DECODE_ID(hw), 0, "dec_dpb_status error %x\n", dec_dpb_status);
			WRITE_VREG(DPB_STATUS_REG, 0);
			return IRQ_HANDLED;
		}
	}
	...
	if (dec_dpb_status == H264_CONFIG_REQUEST) {
		...
		if ((hw->error_proc_policy & 0x10000) &&
			...
			if (first_mb_in_slice == decode_mb_count &&
				...
				dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
					"multi_header_error_frame set, cnt = %d, rp %x\n",
		...
		video_signal = (data_high << 16) | data_low;
		...
		if (is_vdec_hevc_combine()) {
			if (slice_header_process_status == 1) {
				if (READ_VREG(MBY_MBX))
					h264_idle_axi_reset(hw, "new_pic_err",
						RESET_IQIDCT_ERR | RESET_MC_ERR | RESET_DBLK_ERR | RESET_PIC_DC_ERR);
				else
					h264_idle_axi_reset(hw, "new_pic",
						RESET_IQIDCT_INI | RESET_MC_INI | RESET_DBLK_INI);
			}
		}
		...
		if (p_H264_Dpb->mVideo.dec_picture) {
			...
			if (slice_header_process_status == 1) {
				...
				hw->data_flag = I_flag;
				if ((p_H264_Dpb->dpb_param.dpb.NAL_info_mmco & 0x1f) == 5)
			...
			cfg_ret = config_decode_buf(hw, p_H264_Dpb->mVideo.dec_picture);
...
pic_done_proc:
		...
		if ((dec_dpb_status == H264_SEARCH_BUFEMPTY) ||
			...
			if (hw->dpb.mVideo.dec_picture == NULL)
				vdec_v4l_post_error_frame_event(ctx);
		}
		...
		if (hw->frmbase_cont_flag) {
			...
			if (one_packet_multi_frames_multi_run) {
				...
				hw->reserved_byte = bitcnt >> 3;
				hw->consume_byte = hw->chunk_size - hw->reserved_byte;
		...
		if (READ_VREG(H264_AUX_DATA_SIZE) != 0) {
			...
			if (vdec_frame_based(vdec)) {
				if (hw->last_dec_picture)
					set_aux_data(hw, hw->last_dec_picture, 0, 0, NULL);
				...
				if (vdec->master) {
					...
					if (hw_bl->last_dec_picture != NULL) {
						set_aux_data(hw_bl, hw_bl->last_dec_picture, 0, 1, hw);
					}
					set_aux_data(hw, hw->last_dec_picture, 0, 2, NULL);
				}
			}
#else
			if (hw->last_dec_picture)
...
send_again:
		...
		if (aux_data_len != 0) {
			...
			dpb_print(DECODE_ID(hw), PRINT_FLAG_SEI_DETAIL,
				...
				__func__, hw->sei_data_len, aux_data_len);
			...
			if (trans_data_buf[7] == AUX_TAG_SEI) {
				...
				if ((first_mb_in_slice == 0) && (hw->sei_need_parse == false)) {
					hw->sei_data_len = 0;
					hw->sei_itu_data_len = 0;
				}
				...
				if (aux_data_len/2 <= left_len) {
					...
					if (dpb_is_debug(DECODE_ID(hw), PRINT_FLAG_SEI_DETAIL)) {
						u8 *tmp = hw->sei_data_buf;
						dpb_print(DECODE_ID(hw), PRINT_FLAG_SEI_DETAIL,
							"%s: sei_data_len: %d, \n",	__func__, hw->sei_data_len);
						for (i = 0; i < hw->sei_data_len; i++) {
							dpb_print_cont(DECODE_ID(hw), 0, "%02x ", tmp[i]);
							if (((i + 1) & 0xf) == 0)
								dpb_print_cont(DECODE_ID(hw), 0, "\n");
						}
						dpb_print_cont(DECODE_ID(hw), 0, "\n");
...
static void timeout_process(struct vdec_h264_hw_s *hw)
{
	...
	dpb_print(DECODE_ID(hw),
		PRINT_FLAG_ERROR, "%s decoder timeout, pc 0x%x, status 0x%x\n",
		__func__, READ_VREG(MPC_E), READ_VREG(DPB_STATUS_REG));
...
{
	int i;
	dpb_print(DECODE_ID(hw), 0, "%s in %s:\n", __func__, caller);
	for (i = 0; i < BUFSPEC_POOL_SIZE; i++) {
		if (hw->buffer_spec[i].used == -1)
			continue;
		...
		dpb_print_cont(DECODE_ID(hw), 0,
			"dv_el_exist %d", hw->buffer_spec[i].dv_enhance_exist
		);
#endif
		dpb_print_cont(DECODE_ID(hw), 0, "\n");
	}
...
static void vmh264_dump_state(struct vdec_s *vdec)
{
	...
	if (input_frame_based(vdec) &&
		dpb_is_debug(DECODE_ID(hw), PRINT_FRAMEBASE_DATA)) {
		int jj;
...
static void vh264_local_init(struct vdec_h264_hw_s *hw, int flush_flag, bool is_reset)
{
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL,
			"sync_outside=%d, use_idr_framerate=%d, is_used_v4l: %d\n",
...
static void ucode_copy(void *mc_cpu_addr, char *data)
{
	/*header*/
	...
	/*main*/
	memcpy((u8 *)mc_cpu_addr + MC_OFFSET_MAIN,
		data, 0x2000);
	/*data*/
...
static void vmh264_destroy_userdata_manager(struct vdec_h264_hw_s *hw)
{
	if (hw)
		memset(&hw->userdata_info,
			0, sizeof(struct mh264_userdata_info_t));
}
...
int set_mmu_config(struct vdec_h264_hw_s *hw, struct vdec_s *vdec)
{
...

	{
		...
		if (is_vdec_hevc_combine() && !fw_tee_enabled() && hw->mc_cpu_addr) {
			/* -- ucode loading (amrisc and swap code) */
...
static int clear_mmu_config(struct vdec_h264_hw_s *hw, struct vdec_s *vdec)
{
	...
	if (is_vdec_hevc_combine() && !fw_tee_enabled() && hw->mc_cpu_addr) {
		/* -- ucode loading (amrisc and swap code) */
		ucode_copy(hw->mc_cpu_addr, hw->fw->data);
	}
...
static int h264_max_mmu_buf_size(int max_w, int max_h)
{
	int buf_size = 64;
...
static void h264_set_comp_info(struct aml_vcodec_ctx *ctx, struct aml_vdec_ps_infos *ps)
{
	struct vdec_comp_buf_info info;

	h264_get_comp_buf_info(ps, &info);
	...
	u32 param1, u32 param2, u32 param3, u32 param4)
{
	...
	if (hw->dpb.dec_dpb_size != 0) {
		dec_dpb_size = get_dec_dpb_size_active(hw, param1, param4);
		dec_dpb_size_change = hw->dpb.dec_dpb_size!= dec_dpb_size;
	}
	...
	if (ctx->param_sets_from_ucode &&
		...
		if (((param1 != 0 &&
			...
			hw->dpb.size_for_res_change = dec_dpb_size;
			hw->dpb.param1_for_res_change = param1;
...
static bool v4l_resolution_double_check(struct vdec_h264_hw_s *hw,
					u32 param1, u32 param4)
{
	struct aml_vcodec_ctx *ctx = (struct aml_vcodec_ctx *)(hw->v4l2_ctx);
	struct h264_dpb_stru *p_H264_Dpb = &hw->dpb;
	...
	if (ctx->resolution_event_done &&
		p_H264_Dpb->param1_for_res_change != 0 &&
		hw->res_ch_flag &&
		((param1 != 0 &&
		(p_H264_Dpb->param1_for_res_change & 0x80ffffff) != (param1 & 0x80ffffff)) ||
		(size_for_check != 0 && p_H264_Dpb->size_for_res_change != 0 &&
		(size_for_check != p_H264_Dpb->size_for_res_change)))) {
		dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL,
				"Rosolution double check fail!"
				"param1_for_check: 0x%x, param1_for_res_change: 0x%x "
				"size_for_check: %d, size_for_res_change: %d\n",
				param1, p_H264_Dpb->param1_for_res_change,
				size_for_check,
				p_H264_Dpb->size_for_res_change);
		hw->res_ch_flag = 0;
		hw->dpb.dec_dpb_size = p_H264_Dpb->size_for_res_change;
		ret = false;
	}

	p_H264_Dpb->size_for_res_change = 0;
		...
		if (ctx->param_sets_from_ucode) {
			if (!v4l_res_change(hw, param1, param2, param3, param4)) {
				if (!hw->v4l_params_parsed) {
					...
					if (v4l_resolution_double_check(hw, param1, param4)) {
						...
						if (hw->res_ch_flag) {
							...
							dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL, "%s %d param1:%px param2:%px param3:%px param4:%px\n",
								__func__, __LINE__, param1, param2, param3, param4);
	...
	void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	if ((vdec_frame_based(vdec)) &&
		...
		dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL,
			"%s: DEC_RESULT_UNFINISH, after, chunk_offset:0x%x, chunk_size:0x%x, data_invalid:0x%x\n",
			__func__,
			hw->chunk_offset,
			hw->chunk_size,
			data_invalid);
		size = hw->chunk_size;
		hw->consume_byte = 0; // must reset to 0, avoid res change 3 times run with different data size.
		WRITE_VREG(AV_SCRATCH_1, hw->reserved_byte*8); // reuse the register AV_SCRATCH_1 to store the reserved bit_cnt for one packet multi-frame
	} else {
	...
	if (!hw->mmu_enable && is_vdec_hevc_combine()) {
		...
		WRITE_VREG(HEVCD_IPP_TOP_FRMCONFIG,
			((hw->mb_height_hevc << 4) & 0xffff) << 16 |
			((hw->mb_width_hevc << 4) & 0xffff));

		// config fix stride
		WRITE_VREG(HEVCD_MCR_FIXSIZE_CFG, ((1 << 15) | (hw->mb_width << 4)));
...
static void reset(struct vdec_s *vdec)
{
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL, "vmh264 reset\n");
...
static void h264_reset_bufmgr_v4l(struct vdec_s *vdec, int flush_flag, bool reset_flags)
{
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL, "%s frame count %d to skip %d\n\n",
	...
	if (flush_flag) {
		...
		if (vh264_set_params(hw,
			...
			hw->cfg_param2,
			hw->cfg_param3,
			hw->cfg_param4, hw->reset_bufmgr_flag, reset_flags) < 0)
			hw->stat |= DECODER_FATAL_ERROR_SIZE_OVERFLOW;
		else
			hw->stat &= (~DECODER_FATAL_ERROR_SIZE_OVERFLOW);
	}
...
static int ammvdec_h264_probe(struct platform_device *pdev)
{
	...
	if (pdata->config_len) {
		...
		if (get_config_int(pdata->config,
			...
			hw->high_bandwidth_flag = hw->metadata_config_flag & VDEC_CFG_FLAG_HIGH_BANDWIDTH;
	...
	dpb_print(DECODE_ID(hw), PRINT_FLAG_DEC_DETAIL, "ammvdec_h264 mem-addr=%lx,buff_offset=%x,buf_start=%lx\n",
...
{
	dpb_print(0, PRINT_FLAG_DEC_DETAIL,
	...
	vcodec_profile_register_v2("H.264-V4L", VFORMAT_H264, 1);
